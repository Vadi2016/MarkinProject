"use client"
import * as h3 from "h3-js";
import React, {useContext, useState} from "react";

const MyContext = React.createContext(
    {
        isShowModal: false,
        isFilterByTemp: false,
        isShowList: false,
        isFilterByCost: false,
        isFilterByPrecipitation: false,
        filterByGrade: [],
        geo:  {"geo": []
        }
    }
);

export function useMyContext() {
    return useContext(MyContext);
}

export function MyProvider({ children }) {
    const [isShowModal, setIsShowModal] = useState(false);
    const [isShowList, setIsShowList] = useState(false);
    const [isFilterByTemp, setIsFilterByTemp] = useState(false);
    const [isFilterByCost, setIsFilterByCost] = useState(false);
    const [isFilterByPrecipitation, setIsFilterByPrecipitation] = useState(false);
    const [filterByGrade, setFilterByGrade] = useState([]);
    const [geo, setGeo] = useState();

    const showModal = () => {
        setIsShowModal(isShowModal => !isShowModal)
    }

    const showFilterByTemp = () => {
        setIsFilterByTemp(isFilterByTemp => !isFilterByTemp)
    }

    const showList = () => {
        setIsShowList(isShowList => !isShowList)
    }

    const showFilterByCost = () => {
        setIsFilterByCost(isFilterByCost => !isFilterByCost)
    }

    const showFilterByPrecipitation = () => {
        setIsFilterByPrecipitation(isFilterByPrecipitation => !isFilterByPrecipitation)
    }

    const addToFilterByGrade = (value) => {
        setFilterByGrade(value)
    }

    const addGeo = (newGeo) => {
        const {geoH3} = convertGeoJsonToH3(newGeo)
        setGeo(geoH3)
    }

    const addGeoWithout = (newGeo) => {
        setGeo(newGeo)
    }

    return (
        <MyContext.Provider value={{
            isShowModal,
            isFilterByTemp,
            isShowList,
            isFilterByCost,
            isFilterByPrecipitation,
            filterByGrade,
            geo,
            showModal,
            showFilterByTemp,
            showList,
            showFilterByCost,
            showFilterByPrecipitation,
            addToFilterByGrade,
            addGeo,
            addGeoWithout,
        }}>
            {children}
        </MyContext.Provider>
    );
}


const getWeight = (count, min, max) => {
    if (count > 0) {
        let op = Math.round((10 * (count - min)) / (max - min)) / 10;
        return op > 0.1 ? op : 0.1;
    } else return 0.1;
};

const groupPolygonsByOpacity = (polygons) => {
    const retMap = {}; // e.g {0.1: [hex1, hex2]}
    polygons.forEach(([pol, weight]) => {
        if (retMap['coordinates']) {
            retMap['coordinates'] = [...retMap[weight.toString()], pol];
        } else {
            retMap['coordinates'] = [pol];
        }
    });
    return retMap;
};

export function convertGeoJsonToH3 () {
    const geo = {
        "geo": [
            {
                "level_0": 0,
                "index": 0,
                "h3": "8310acfffffffff",
                "val": 1.0,
                "color": "#7f0000"
            },
            {
                "level_0": 1,
                "index": 1,
                "h3": "83118afffffffff",
                "val": 0.964814028,
                "color": "#a80000"
            },
            {
                "level_0": 2,
                "index": 2,
                "h3": "831191fffffffff",
                "val": 0.9079602951,
                "color": "#e80000"
            },
            {
                "level_0": 3,
                "index": 3,
                "h3": "8311aafffffffff",
                "val": 0.896952609,
                "color": "#f50b00"
            },
            {
                "level_0": 4,
                "index": 4,
                "h3": "831198fffffffff",
                "val": 0.8653199954,
                "color": "#ff2800"
            },
            {
                "level_0": 5,
                "index": 5,
                "h3": "831193fffffffff",
                "val": 0.8028658607,
                "color": "#ff6400"
            },
            {
                "level_0": 6,
                "index": 6,
                "h3": "831106fffffffff",
                "val": 0.775597698,
                "color": "#ff7e00"
            },
            {
                "level_0": 7,
                "index": 7,
                "h3": "831181fffffffff",
                "val": 0.7559769804,
                "color": "#ff9000"
            },
            {
                "level_0": 8,
                "index": 8,
                "h3": "8310a4fffffffff",
                "val": 0.7499517207,
                "color": "#ff9800"
            },
            {
                "level_0": 9,
                "index": 9,
                "h3": "830bb0fffffffff",
                "val": 0.717817002,
                "color": "#ffb500"
            },
            {
                "level_0": 10,
                "index": 10,
                "h3": "831081fffffffff",
                "val": 0.6944498088,
                "color": "#ffcb00"
            },
            {
                "level_0": 11,
                "index": 11,
                "h3": "832d5dfffffffff",
                "val": 0.6812405855,
                "color": "#ffd700"
            },
            {
                "level_0": 12,
                "index": 12,
                "h3": "831036fffffffff",
                "val": 0.6801205052,
                "color": "#ffd700"
            },
            {
                "level_0": 13,
                "index": 13,
                "h3": "83119cfffffffff",
                "val": 0.663898652,
                "color": "#ffe900"
            },
            {
                "level_0": 14,
                "index": 14,
                "h3": "83108dfffffffff",
                "val": 0.6430806072,
                "color": "#f1fc05"
            },
            {
                "level_0": 15,
                "index": 15,
                "h3": "832d2dfffffffff",
                "val": 0.6383685451,
                "color": "#edff08"
            },
            {
                "level_0": 16,
                "index": 16,
                "h3": "831190fffffffff",
                "val": 0.6318411803,
                "color": "#e7ff0f"
            },
            {
                "level_0": 17,
                "index": 17,
                "h3": "830b06fffffffff",
                "val": 0.6309142173,
                "color": "#e7ff0f"
            },
            {
                "level_0": 18,
                "index": 18,
                "h3": "831183fffffffff",
                "val": 0.6296396431,
                "color": "#e7ff0f"
            },
            {
                "level_0": 19,
                "index": 19,
                "h3": "83119efffffffff",
                "val": 0.6058475918,
                "color": "#d4ff22"
            },
            {
                "level_0": 20,
                "index": 20,
                "h3": "831199fffffffff",
                "val": 0.604766135,
                "color": "#d0ff25"
            },
            {
                "level_0": 21,
                "index": 21,
                "h3": "83118dfffffffff",
                "val": 0.6026418447,
                "color": "#d0ff25"
            },
            {
                "level_0": 22,
                "index": 22,
                "h3": "831085fffffffff",
                "val": 0.6003244371,
                "color": "#cdff29"
            },
            {
                "level_0": 23,
                "index": 23,
                "h3": "832d48fffffffff",
                "val": 0.5980456529,
                "color": "#cdff29"
            },
            {
                "level_0": 24,
                "index": 24,
                "h3": "8310abfffffffff",
                "val": 0.5901278437,
                "color": "#c7ff2f"
            },
            {
                "level_0": 25,
                "index": 25,
                "h3": "8310f6fffffffff",
                "val": 0.5627824341,
                "color": "#b0ff46"
            },
            {
                "level_0": 26,
                "index": 26,
                "h3": "832c29fffffffff",
                "val": 0.5601946622,
                "color": "#adff49"
            },
            {
                "level_0": 27,
                "index": 27,
                "h3": "832d44fffffffff",
                "val": 0.5548260013,
                "color": "#aaff4c"
            },
            {
                "level_0": 28,
                "index": 28,
                "h3": "831182fffffffff",
                "val": 0.550268433,
                "color": "#a3ff53"
            },
            {
                "level_0": 29,
                "index": 29,
                "h3": "83206afffffffff",
                "val": 0.546985439,
                "color": "#a3ff53"
            },
            {
                "level_0": 30,
                "index": 30,
                "h3": "83118cfffffffff",
                "val": 0.5415395311,
                "color": "#9dff59"
            },
            {
                "level_0": 31,
                "index": 31,
                "h3": "8310a6fffffffff",
                "val": 0.5253563014,
                "color": "#90ff66"
            },
            {
                "level_0": 32,
                "index": 32,
                "h3": "8310dcfffffffff",
                "val": 0.5234637519,
                "color": "#90ff66"
            },
            {
                "level_0": 33,
                "index": 33,
                "h3": "831016fffffffff",
                "val": 0.5192151713,
                "color": "#89ff6c"
            },
            {
                "level_0": 34,
                "index": 34,
                "h3": "831195fffffffff",
                "val": 0.514155498,
                "color": "#86ff70"
            },
            {
                "level_0": 35,
                "index": 35,
                "h3": "831032fffffffff",
                "val": 0.4977019041,
                "color": "#79ff7d"
            },
            {
                "level_0": 36,
                "index": 36,
                "h3": "832d45fffffffff",
                "val": 0.4960410954,
                "color": "#76ff80"
            },
            {
                "level_0": 37,
                "index": 37,
                "h3": "832d6dfffffffff",
                "val": 0.4947278977,
                "color": "#76ff80"
            },
            {
                "level_0": 38,
                "index": 38,
                "h3": "830bb2fffffffff",
                "val": 0.491560774,
                "color": "#73ff83"
            },
            {
                "level_0": 39,
                "index": 39,
                "h3": "831188fffffffff",
                "val": 0.4831794832,
                "color": "#6cff89"
            },
            {
                "level_0": 40,
                "index": 40,
                "h3": "831186fffffffff",
                "val": 0.4810551929,
                "color": "#6cff89"
            },
            {
                "level_0": 41,
                "index": 41,
                "h3": "832141fffffffff",
                "val": 0.4711675872,
                "color": "#63ff93"
            },
            {
                "level_0": 42,
                "index": 42,
                "h3": "8311a8fffffffff",
                "val": 0.4642539879,
                "color": "#5cff9a"
            },
            {
                "level_0": 43,
                "index": 43,
                "h3": "831082fffffffff",
                "val": 0.4559113205,
                "color": "#56ffa0"
            },
            {
                "level_0": 44,
                "index": 44,
                "h3": "831185fffffffff",
                "val": 0.4523965857,
                "color": "#53ffa3"
            },
            {
                "level_0": 45,
                "index": 45,
                "h3": "831080fffffffff",
                "val": 0.4431269553,
                "color": "#4cffaa"
            },
            {
                "level_0": 46,
                "index": 46,
                "h3": "831192fffffffff",
                "val": 0.4410799119,
                "color": "#49ffad"
            },
            {
                "level_0": 47,
                "index": 47,
                "h3": "8310f0fffffffff",
                "val": 0.4394963501,
                "color": "#49ffad"
            },
            {
                "level_0": 48,
                "index": 48,
                "h3": "830b86fffffffff",
                "val": 0.4351705226,
                "color": "#46ffb0"
            },
            {
                "level_0": 49,
                "index": 49,
                "h3": "831194fffffffff",
                "val": 0.4344366768,
                "color": "#46ffb0"
            },
            {
                "level_0": 50,
                "index": 50,
                "h3": "8310aefffffffff",
                "val": 0.4323123865,
                "color": "#42ffb3"
            },
            {
                "level_0": 51,
                "index": 51,
                "h3": "8311abfffffffff",
                "val": 0.4307674481,
                "color": "#42ffb3"
            },
            {
                "level_0": 52,
                "index": 52,
                "h3": "832123fffffffff",
                "val": 0.4296473678,
                "color": "#3fffb7"
            },
            {
                "level_0": 53,
                "index": 53,
                "h3": "8310a8fffffffff",
                "val": 0.4184465644,
                "color": "#39ffbd"
            },
            {
                "level_0": 54,
                "index": 54,
                "h3": "8311aefffffffff",
                "val": 0.4180989533,
                "color": "#39ffbd"
            },
            {
                "level_0": 55,
                "index": 55,
                "h3": "8310aafffffffff",
                "val": 0.417403731,
                "color": "#36ffc0"
            },
            {
                "level_0": 56,
                "index": 56,
                "h3": "831099fffffffff",
                "val": 0.4107604959,
                "color": "#32ffc3"
            },
            {
                "level_0": 57,
                "index": 57,
                "h3": "832d6cfffffffff",
                "val": 0.4081341007,
                "color": "#2fffc7"
            },
            {
                "level_0": 58,
                "index": 58,
                "h3": "83119afffffffff",
                "val": 0.4021088409,
                "color": "#29ffcd"
            },
            {
                "level_0": 59,
                "index": 59,
                "h3": "8310c2fffffffff",
                "val": 0.400100421,
                "color": "#29ffcd"
            },
            {
                "level_0": 60,
                "index": 60,
                "h3": "831086fffffffff",
                "val": 0.3977443899,
                "color": "#25ffd0"
            },
            {
                "level_0": 61,
                "index": 61,
                "h3": "8310a3fffffffff",
                "val": 0.3922598586,
                "color": "#22ffd4"
            },
            {
                "level_0": 62,
                "index": 62,
                "h3": "832d40fffffffff",
                "val": 0.3853076359,
                "color": "#1cffda"
            },
            {
                "level_0": 63,
                "index": 63,
                "h3": "832063fffffffff",
                "val": 0.3778146846,
                "color": "#15ffe1"
            },
            {
                "level_0": 64,
                "index": 64,
                "h3": "832d73fffffffff",
                "val": 0.3717894249,
                "color": "#12fce4"
            },
            {
                "level_0": 65,
                "index": 65,
                "h3": "8310a0fffffffff",
                "val": 0.3691630296,
                "color": "#0ff8e7"
            },
            {
                "level_0": 66,
                "index": 66,
                "h3": "831104fffffffff",
                "val": 0.3671932332,
                "color": "#0ff8e7"
            },
            {
                "level_0": 67,
                "index": 67,
                "h3": "8311b1fffffffff",
                "val": 0.3591595535,
                "color": "#05ecf1"
            },
            {
                "level_0": 68,
                "index": 68,
                "h3": "83119dfffffffff",
                "val": 0.3556448187,
                "color": "#05ecf1"
            },
            {
                "level_0": 69,
                "index": 69,
                "h3": "8310f1fffffffff",
                "val": 0.3522459542,
                "color": "#02e8f4"
            },
            {
                "level_0": 70,
                "index": 70,
                "h3": "831189fffffffff",
                "val": 0.3506623923,
                "color": "#00e4f7"
            },
            {
                "level_0": 71,
                "index": 71,
                "h3": "831180fffffffff",
                "val": 0.350469275,
                "color": "#00e4f7"
            },
            {
                "level_0": 72,
                "index": 72,
                "h3": "832d58fffffffff",
                "val": 0.3425128423,
                "color": "#00dcfe"
            },
            {
                "level_0": 73,
                "index": 73,
                "h3": "832571fffffffff",
                "val": 0.3406202928,
                "color": "#00dcfe"
            },
            {
                "level_0": 74,
                "index": 74,
                "h3": "8311b3fffffffff",
                "val": 0.3403499285,
                "color": "#00dcfe"
            },
            {
                "level_0": 75,
                "index": 75,
                "h3": "8311a9fffffffff",
                "val": 0.3402726816,
                "color": "#00dcfe"
            },
            {
                "level_0": 76,
                "index": 76,
                "h3": "83118efffffffff",
                "val": 0.3367965702,
                "color": "#00d8ff"
            },
            {
                "level_0": 77,
                "index": 77,
                "h3": "8310f3fffffffff",
                "val": 0.3351743849,
                "color": "#00d4ff"
            },
            {
                "level_0": 78,
                "index": 78,
                "h3": "832d5efffffffff",
                "val": 0.3347881503,
                "color": "#00d4ff"
            },
            {
                "level_0": 79,
                "index": 79,
                "h3": "8310b1fffffffff",
                "val": 0.3313892859,
                "color": "#00d0ff"
            },
            {
                "level_0": 80,
                "index": 80,
                "h3": "832105fffffffff",
                "val": 0.3289560079,
                "color": "#00d0ff"
            },
            {
                "level_0": 81,
                "index": 81,
                "h3": "8310defffffffff",
                "val": 0.3289173844,
                "color": "#00d0ff"
            },
            {
                "level_0": 82,
                "index": 82,
                "h3": "832c2dfffffffff",
                "val": 0.3217720443,
                "color": "#00c8ff"
            },
            {
                "level_0": 83,
                "index": 83,
                "h3": "8311a3fffffffff",
                "val": 0.3177552045,
                "color": "#00c4ff"
            },
            {
                "level_0": 84,
                "index": 84,
                "h3": "832068fffffffff",
                "val": 0.3162488896,
                "color": "#00c0ff"
            },
            {
                "level_0": 85,
                "index": 85,
                "h3": "831e4bfffffffff",
                "val": 0.3118844386,
                "color": "#00bcff"
            },
            {
                "level_0": 86,
                "index": 86,
                "h3": "830b31fffffffff",
                "val": 0.3118458151,
                "color": "#00bcff"
            },
            {
                "level_0": 87,
                "index": 87,
                "h3": "83108cfffffffff",
                "val": 0.3087945618,
                "color": "#00bcff"
            },
            {
                "level_0": 88,
                "index": 88,
                "h3": "832d68fffffffff",
                "val": 0.3056274381,
                "color": "#00b8ff"
            },
            {
                "level_0": 89,
                "index": 89,
                "h3": "832d4dfffffffff",
                "val": 0.2990614499,
                "color": "#00b0ff"
            },
            {
                "level_0": 90,
                "index": 90,
                "h3": "8310a2fffffffff",
                "val": 0.2982117338,
                "color": "#00b0ff"
            },
            {
                "level_0": 91,
                "index": 91,
                "h3": "83119bfffffffff",
                "val": 0.2976323819,
                "color": "#00b0ff"
            },
            {
                "level_0": 92,
                "index": 92,
                "h3": "830b04fffffffff",
                "val": 0.295121857,
                "color": "#00acff"
            },
            {
                "level_0": 93,
                "index": 93,
                "h3": "831196fffffffff",
                "val": 0.2864315785,
                "color": "#00a4ff"
            },
            {
                "level_0": 94,
                "index": 94,
                "h3": "832d43fffffffff",
                "val": 0.2837665598,
                "color": "#00a0ff"
            },
            {
                "level_0": 95,
                "index": 95,
                "h3": "832d41fffffffff",
                "val": 0.2830327141,
                "color": "#00a0ff"
            },
            {
                "level_0": 96,
                "index": 96,
                "h3": "830bb4fffffffff",
                "val": 0.2828009733,
                "color": "#00a0ff"
            },
            {
                "level_0": 97,
                "index": 97,
                "h3": "830bb3fffffffff",
                "val": 0.2824919856,
                "color": "#00a0ff"
            },
            {
                "level_0": 98,
                "index": 98,
                "h3": "83214cfffffffff",
                "val": 0.2795952261,
                "color": "#009cff"
            },
            {
                "level_0": 99,
                "index": 99,
                "h3": "832d29fffffffff",
                "val": 0.2774323124,
                "color": "#009cff"
            },
            {
                "level_0": 100,
                "index": 100,
                "h3": "830b02fffffffff",
                "val": 0.2767370901,
                "color": "#0098ff"
            },
            {
                "level_0": 101,
                "index": 101,
                "h3": "831f73fffffffff",
                "val": 0.2765825963,
                "color": "#0098ff"
            },
            {
                "level_0": 102,
                "index": 102,
                "h3": "832d5cfffffffff",
                "val": 0.2750762813,
                "color": "#0098ff"
            },
            {
                "level_0": 103,
                "index": 103,
                "h3": "832d4cfffffffff",
                "val": 0.2742651887,
                "color": "#0098ff"
            },
            {
                "level_0": 104,
                "index": 104,
                "h3": "831083fffffffff",
                "val": 0.2739948245,
                "color": "#0098ff"
            },
            {
                "level_0": 105,
                "index": 105,
                "h3": "832526fffffffff",
                "val": 0.2724498861,
                "color": "#0094ff"
            },
            {
                "level_0": 106,
                "index": 106,
                "h3": "83118bfffffffff",
                "val": 0.2719864045,
                "color": "#0094ff"
            },
            {
                "level_0": 107,
                "index": 107,
                "h3": "832d42fffffffff",
                "val": 0.2711366884,
                "color": "#0094ff"
            },
            {
                "level_0": 108,
                "index": 108,
                "h3": "83206dfffffffff",
                "val": 0.2707504538,
                "color": "#0094ff"
            },
            {
                "level_0": 109,
                "index": 109,
                "h3": "83216afffffffff",
                "val": 0.2704414661,
                "color": "#0094ff"
            },
            {
                "level_0": 110,
                "index": 110,
                "h3": "831184fffffffff",
                "val": 0.2696689969,
                "color": "#0094ff"
            },
            {
                "level_0": 111,
                "index": 111,
                "h3": "830b11fffffffff",
                "val": 0.2659611448,
                "color": "#0090ff"
            },
            {
                "level_0": 112,
                "index": 112,
                "h3": "832d69fffffffff",
                "val": 0.2650728052,
                "color": "#008cff"
            },
            {
                "level_0": 113,
                "index": 113,
                "h3": "83214bfffffffff",
                "val": 0.2642230891,
                "color": "#008cff"
            },
            {
                "level_0": 114,
                "index": 114,
                "h3": "8310d3fffffffff",
                "val": 0.2641072187,
                "color": "#008cff"
            },
            {
                "level_0": 115,
                "index": 115,
                "h3": "831088fffffffff",
                "val": 0.2620215519,
                "color": "#008cff"
            },
            {
                "level_0": 116,
                "index": 116,
                "h3": "832149fffffffff",
                "val": 0.2598972616,
                "color": "#0088ff"
            },
            {
                "level_0": 117,
                "index": 117,
                "h3": "832c64fffffffff",
                "val": 0.2586999343,
                "color": "#0088ff"
            },
            {
                "level_0": 118,
                "index": 118,
                "h3": "832126fffffffff",
                "val": 0.2540651192,
                "color": "#0084ff"
            },
            {
                "level_0": 119,
                "index": 119,
                "h3": "832d53fffffffff",
                "val": 0.2509752424,
                "color": "#0080ff"
            },
            {
                "level_0": 120,
                "index": 120,
                "h3": "832d72fffffffff",
                "val": 0.246997026,
                "color": "#007cff"
            },
            {
                "level_0": 121,
                "index": 121,
                "h3": "8310b2fffffffff",
                "val": 0.2461086864,
                "color": "#007cff"
            },
            {
                "level_0": 122,
                "index": 122,
                "h3": "8310d2fffffffff",
                "val": 0.2457224518,
                "color": "#0078ff"
            },
            {
                "level_0": 123,
                "index": 123,
                "h3": "83214dfffffffff",
                "val": 0.2425553281,
                "color": "#0078ff"
            },
            {
                "level_0": 124,
                "index": 124,
                "h3": "8311b0fffffffff",
                "val": 0.2416669885,
                "color": "#0074ff"
            },
            {
                "level_0": 125,
                "index": 125,
                "h3": "830bb6fffffffff",
                "val": 0.2416283651,
                "color": "#0074ff"
            },
            {
                "level_0": 126,
                "index": 126,
                "h3": "832575fffffffff",
                "val": 0.2398516859,
                "color": "#0074ff"
            },
            {
                "level_0": 127,
                "index": 127,
                "h3": "832d6afffffffff",
                "val": 0.2390792167,
                "color": "#0074ff"
            },
            {
                "level_0": 128,
                "index": 128,
                "h3": "832d51fffffffff",
                "val": 0.2377273956,
                "color": "#0070ff"
            },
            {
                "level_0": 129,
                "index": 129,
                "h3": "83206bfffffffff",
                "val": 0.2373797845,
                "color": "#0070ff"
            },
            {
                "level_0": 130,
                "index": 130,
                "h3": "8311a2fffffffff",
                "val": 0.2371480437,
                "color": "#0070ff"
            },
            {
                "level_0": 131,
                "index": 131,
                "h3": "832122fffffffff",
                "val": 0.2370321733,
                "color": "#0070ff"
            },
            {
                "level_0": 132,
                "index": 132,
                "h3": "8310a9fffffffff",
                "val": 0.2364914449,
                "color": "#0070ff"
            },
            {
                "level_0": 133,
                "index": 133,
                "h3": "83109dfffffffff",
                "val": 0.2363369511,
                "color": "#0070ff"
            },
            {
                "level_0": 134,
                "index": 134,
                "h3": "832d46fffffffff",
                "val": 0.2347533892,
                "color": "#0070ff"
            },
            {
                "level_0": 135,
                "index": 135,
                "h3": "8310f4fffffffff",
                "val": 0.2338264262,
                "color": "#006cff"
            },
            {
                "level_0": 136,
                "index": 136,
                "h3": "832c76fffffffff",
                "val": 0.2326677224,
                "color": "#006cff"
            },
            {
                "level_0": 137,
                "index": 137,
                "h3": "831014fffffffff",
                "val": 0.2324359816,
                "color": "#006cff"
            },
            {
                "level_0": 138,
                "index": 138,
                "h3": "8310b3fffffffff",
                "val": 0.231624889,
                "color": "#006cff"
            },
            {
                "level_0": 139,
                "index": 139,
                "h3": "8310f2fffffffff",
                "val": 0.2314317717,
                "color": "#006cff"
            },
            {
                "level_0": 140,
                "index": 140,
                "h3": "8311a0fffffffff",
                "val": 0.2307751728,
                "color": "#006cff"
            },
            {
                "level_0": 141,
                "index": 141,
                "h3": "8311adfffffffff",
                "val": 0.2237843266,
                "color": "#0064ff"
            },
            {
                "level_0": 142,
                "index": 142,
                "h3": "832d4afffffffff",
                "val": 0.2231277278,
                "color": "#0064ff"
            },
            {
                "level_0": 143,
                "index": 143,
                "h3": "83108efffffffff",
                "val": 0.2210420609,
                "color": "#0060ff"
            },
            {
                "level_0": 144,
                "index": 144,
                "h3": "830b9afffffffff",
                "val": 0.2202695918,
                "color": "#0060ff"
            },
            {
                "level_0": 145,
                "index": 145,
                "h3": "832d4efffffffff",
                "val": 0.2192267583,
                "color": "#0060ff"
            },
            {
                "level_0": 146,
                "index": 146,
                "h3": "832d4bfffffffff",
                "val": 0.2184929126,
                "color": "#005cff"
            },
            {
                "level_0": 147,
                "index": 147,
                "h3": "830b94fffffffff",
                "val": 0.2181839249,
                "color": "#005cff"
            },
            {
                "level_0": 148,
                "index": 148,
                "h3": "832148fffffffff",
                "val": 0.2150554247,
                "color": "#005cff"
            },
            {
                "level_0": 149,
                "index": 149,
                "h3": "832060fffffffff",
                "val": 0.2134332393,
                "color": "#0058ff"
            },
            {
                "level_0": 150,
                "index": 150,
                "h3": "831084fffffffff",
                "val": 0.2125062763,
                "color": "#0058ff"
            },
            {
                "level_0": 151,
                "index": 151,
                "h3": "831131fffffffff",
                "val": 0.2116951837,
                "color": "#0058ff"
            },
            {
                "level_0": 152,
                "index": 152,
                "h3": "832d59fffffffff",
                "val": 0.2080645784,
                "color": "#0054ff"
            },
            {
                "level_0": 153,
                "index": 153,
                "h3": "8310adfffffffff",
                "val": 0.2072148623,
                "color": "#0054ff"
            },
            {
                "level_0": 154,
                "index": 154,
                "h3": "8311acfffffffff",
                "val": 0.205090572,
                "color": "#0050ff"
            },
            {
                "level_0": 155,
                "index": 155,
                "h3": "8311a1fffffffff",
                "val": 0.2043953497,
                "color": "#0050ff"
            },
            {
                "level_0": 156,
                "index": 156,
                "h3": "831115fffffffff",
                "val": 0.2038932448,
                "color": "#0050ff"
            },
            {
                "level_0": 157,
                "index": 157,
                "h3": "831584fffffffff",
                "val": 0.2023096829,
                "color": "#004cff"
            },
            {
                "level_0": 158,
                "index": 158,
                "h3": "8310a5fffffffff",
                "val": 0.2016144606,
                "color": "#004cff"
            },
            {
                "level_0": 159,
                "index": 159,
                "h3": "830bb5fffffffff",
                "val": 0.2015372137,
                "color": "#004cff"
            },
            {
                "level_0": 160,
                "index": 160,
                "h3": "8310c3fffffffff",
                "val": 0.2006102507,
                "color": "#004cff"
            },
            {
                "level_0": 161,
                "index": 161,
                "h3": "8310c4fffffffff",
                "val": 0.2001853926,
                "color": "#004cff"
            },
            {
                "level_0": 162,
                "index": 162,
                "h3": "830b80fffffffff",
                "val": 0.1996832876,
                "color": "#004cff"
            },
            {
                "level_0": 163,
                "index": 163,
                "h3": "831f69fffffffff",
                "val": 0.1979452319,
                "color": "#0048ff"
            },
            {
                "level_0": 164,
                "index": 164,
                "h3": "832d6efffffffff",
                "val": 0.1966320343,
                "color": "#0048ff"
            },
            {
                "level_0": 165,
                "index": 165,
                "h3": "83212dfffffffff",
                "val": 0.1963616701,
                "color": "#0048ff"
            },
            {
                "level_0": 166,
                "index": 166,
                "h3": "8310c0fffffffff",
                "val": 0.1957823182,
                "color": "#0048ff"
            },
            {
                "level_0": 167,
                "index": 167,
                "h3": "8311a6fffffffff",
                "val": 0.193078676,
                "color": "#0044ff"
            },
            {
                "level_0": 168,
                "index": 168,
                "h3": "830b84fffffffff",
                "val": 0.1913792438,
                "color": "#0040ff"
            },
            {
                "level_0": 169,
                "index": 169,
                "h3": "831102fffffffff",
                "val": 0.1887528485,
                "color": "#0040ff"
            },
            {
                "level_0": 170,
                "index": 170,
                "h3": "831f6dfffffffff",
                "val": 0.1880962497,
                "color": "#0040ff"
            },
            {
                "level_0": 171,
                "index": 171,
                "h3": "83212cfffffffff",
                "val": 0.187285157,
                "color": "#003cff"
            },
            {
                "level_0": 172,
                "index": 172,
                "h3": "830b95fffffffff",
                "val": 0.1860105828,
                "color": "#003cff"
            },
            {
                "level_0": 173,
                "index": 173,
                "h3": "83111afffffffff",
                "val": 0.1857015951,
                "color": "#003cff"
            },
            {
                "level_0": 174,
                "index": 174,
                "h3": "832d76fffffffff",
                "val": 0.1836931752,
                "color": "#003cff"
            },
            {
                "level_0": 175,
                "index": 175,
                "h3": "83101bfffffffff",
                "val": 0.1832683172,
                "color": "#0038ff"
            },
            {
                "level_0": 176,
                "index": 176,
                "h3": "831018fffffffff",
                "val": 0.1830751999,
                "color": "#0038ff"
            },
            {
                "level_0": 177,
                "index": 177,
                "h3": "83206efffffffff",
                "val": 0.180564675,
                "color": "#0038ff"
            },
            {
                "level_0": 178,
                "index": 178,
                "h3": "831030fffffffff",
                "val": 0.18006257,
                "color": "#0038ff"
            },
            {
                "level_0": 179,
                "index": 179,
                "h3": "8310a1fffffffff",
                "val": 0.1793287243,
                "color": "#0034ff"
            },
            {
                "level_0": 180,
                "index": 180,
                "h3": "831165fffffffff",
                "val": 0.1792901008,
                "color": "#0034ff"
            },
            {
                "level_0": 181,
                "index": 181,
                "h3": "832c60fffffffff",
                "val": 0.1742304276,
                "color": "#0030ff"
            },
            {
                "level_0": 182,
                "index": 182,
                "h3": "8310e3fffffffff",
                "val": 0.1731103472,
                "color": "#0030ff"
            },
            {
                "level_0": 183,
                "index": 183,
                "h3": "830a96fffffffff",
                "val": 0.1726082422,
                "color": "#0030ff"
            },
            {
                "level_0": 184,
                "index": 184,
                "h3": "832d6bfffffffff",
                "val": 0.1695183655,
                "color": "#002cff"
            },
            {
                "level_0": 185,
                "index": 185,
                "h3": "831094fffffffff",
                "val": 0.1687458963,
                "color": "#002cff"
            },
            {
                "level_0": 186,
                "index": 186,
                "h3": "8310d1fffffffff",
                "val": 0.1676644394,
                "color": "#0028ff"
            },
            {
                "level_0": 187,
                "index": 187,
                "h3": "832d54fffffffff",
                "val": 0.1668919702,
                "color": "#0028ff"
            },
            {
                "level_0": 188,
                "index": 188,
                "h3": "8311a4fffffffff",
                "val": 0.1634544823,
                "color": "#0024ff"
            },
            {
                "level_0": 189,
                "index": 189,
                "h3": "8310d9fffffffff",
                "val": 0.1627978834,
                "color": "#0024ff"
            },
            {
                "level_0": 190,
                "index": 190,
                "h3": "8310d6fffffffff",
                "val": 0.1597080066,
                "color": "#0020ff"
            },
            {
                "level_0": 191,
                "index": 191,
                "h3": "831089fffffffff",
                "val": 0.159321772,
                "color": "#0020ff"
            },
            {
                "level_0": 192,
                "index": 192,
                "h3": "8310b0fffffffff",
                "val": 0.1586651732,
                "color": "#0020ff"
            },
            {
                "level_0": 193,
                "index": 193,
                "h3": "832124fffffffff",
                "val": 0.1586651732,
                "color": "#0020ff"
            },
            {
                "level_0": 194,
                "index": 194,
                "h3": "832145fffffffff",
                "val": 0.1585879263,
                "color": "#0020ff"
            },
            {
                "level_0": 195,
                "index": 195,
                "h3": "830b22fffffffff",
                "val": 0.1565022595,
                "color": "#0020ff"
            },
            {
                "level_0": 196,
                "index": 196,
                "h3": "83316cfffffffff",
                "val": 0.1563863891,
                "color": "#0020ff"
            },
            {
                "level_0": 197,
                "index": 197,
                "h3": "83108afffffffff",
                "val": 0.1543007223,
                "color": "#001cff"
            },
            {
                "level_0": 198,
                "index": 198,
                "h3": "831095fffffffff",
                "val": 0.1520605616,
                "color": "#0018ff"
            },
            {
                "level_0": 199,
                "index": 199,
                "h3": "831015fffffffff",
                "val": 0.1513267158,
                "color": "#0018ff"
            },
            {
                "level_0": 200,
                "index": 200,
                "h3": "831113fffffffff",
                "val": 0.1505928701,
                "color": "#0018ff"
            },
            {
                "level_0": 201,
                "index": 201,
                "h3": "831013fffffffff",
                "val": 0.1497045305,
                "color": "#0018ff"
            },
            {
                "level_0": 202,
                "index": 202,
                "h3": "831012fffffffff",
                "val": 0.1496272836,
                "color": "#0018ff"
            },
            {
                "level_0": 203,
                "index": 203,
                "h3": "832d5afffffffff",
                "val": 0.1465374068,
                "color": "#0014ff"
            },
            {
                "level_0": 204,
                "index": 204,
                "h3": "831035fffffffff",
                "val": 0.1450697153,
                "color": "#0014ff"
            },
            {
                "level_0": 205,
                "index": 205,
                "h3": "830b00fffffffff",
                "val": 0.1450310919,
                "color": "#0014ff"
            },
            {
                "level_0": 206,
                "index": 206,
                "h3": "832e88fffffffff",
                "val": 0.1443358696,
                "color": "#0010ff"
            },
            {
                "level_0": 207,
                "index": 207,
                "h3": "83101efffffffff",
                "val": 0.1435247769,
                "color": "#0010ff"
            },
            {
                "level_0": 208,
                "index": 208,
                "h3": "830b93fffffffff",
                "val": 0.1425978139,
                "color": "#0010ff"
            },
            {
                "level_0": 209,
                "index": 209,
                "h3": "8310f5fffffffff",
                "val": 0.1424046966,
                "color": "#0010ff"
            },
            {
                "level_0": 210,
                "index": 210,
                "h3": "831133fffffffff",
                "val": 0.1419025916,
                "color": "#0010ff"
            },
            {
                "level_0": 211,
                "index": 211,
                "h3": "830bb1fffffffff",
                "val": 0.1409370051,
                "color": "#0010ff"
            },
            {
                "level_0": 212,
                "index": 212,
                "h3": "830b9efffffffff",
                "val": 0.1406666409,
                "color": "#0010ff"
            },
            {
                "level_0": 213,
                "index": 213,
                "h3": "8315a2fffffffff",
                "val": 0.1381174926,
                "color": "#000cff"
            },
            {
                "level_0": 214,
                "index": 214,
                "h3": "8310c6fffffffff",
                "val": 0.137731258,
                "color": "#000cff"
            },
            {
                "level_0": 215,
                "index": 215,
                "h3": "832c74fffffffff",
                "val": 0.1362635665,
                "color": "#0008ff"
            },
            {
                "level_0": 216,
                "index": 216,
                "h3": "832d49fffffffff",
                "val": 0.1360318257,
                "color": "#0008ff"
            },
            {
                "level_0": 217,
                "index": 217,
                "h3": "8310b4fffffffff",
                "val": 0.1349503689,
                "color": "#0008ff"
            },
            {
                "level_0": 218,
                "index": 218,
                "h3": "831110fffffffff",
                "val": 0.1346800046,
                "color": "#0008ff"
            },
            {
                "level_0": 219,
                "index": 219,
                "h3": "831150fffffffff",
                "val": 0.1340234058,
                "color": "#0008ff"
            },
            {
                "level_0": 220,
                "index": 220,
                "h3": "831130fffffffff",
                "val": 0.1317832451,
                "color": "#0004ff"
            },
            {
                "level_0": 221,
                "index": 221,
                "h3": "831132fffffffff",
                "val": 0.1307404117,
                "color": "#0004ff"
            },
            {
                "level_0": 222,
                "index": 222,
                "h3": "831114fffffffff",
                "val": 0.130006566,
                "color": "#0004ff"
            },
            {
                "level_0": 223,
                "index": 223,
                "h3": "830b20fffffffff",
                "val": 0.1295430845,
                "color": "#0004ff"
            },
            {
                "level_0": 224,
                "index": 224,
                "h3": "8315a3fffffffff",
                "val": 0.1283457572,
                "color": "#0000ff"
            },
            {
                "level_0": 225,
                "index": 225,
                "h3": "8310ddfffffffff",
                "val": 0.1273029238,
                "color": "#0000ff"
            },
            {
                "level_0": 226,
                "index": 226,
                "h3": "831033fffffffff",
                "val": 0.1221660036,
                "color": "#0000ff"
            },
            {
                "level_0": 227,
                "index": 227,
                "h3": "830a91fffffffff",
                "val": 0.121857016,
                "color": "#0000ff"
            },
            {
                "level_0": 228,
                "index": 228,
                "h3": "832160fffffffff",
                "val": 0.1211231702,
                "color": "#0000ff"
            },
            {
                "level_0": 229,
                "index": 229,
                "h3": "83111cfffffffff",
                "val": 0.1196941022,
                "color": "#0000ff"
            },
            {
                "level_0": 230,
                "index": 230,
                "h3": "83101afffffffff",
                "val": 0.1193851145,
                "color": "#0000ff"
            },
            {
                "level_0": 231,
                "index": 231,
                "h3": "831010fffffffff",
                "val": 0.1193078676,
                "color": "#0000ff"
            },
            {
                "level_0": 232,
                "index": 232,
                "h3": "830a92fffffffff",
                "val": 0.1189988799,
                "color": "#0000ff"
            },
            {
                "level_0": 233,
                "index": 233,
                "h3": "8311b5fffffffff",
                "val": 0.1182264107,
                "color": "#0000ff"
            },
            {
                "level_0": 234,
                "index": 234,
                "h3": "8310d8fffffffff",
                "val": 0.1166814723,
                "color": "#0000ff"
            },
            {
                "level_0": 235,
                "index": 235,
                "h3": "832064fffffffff",
                "val": 0.1146730524,
                "color": "#0000ff"
            },
            {
                "level_0": 236,
                "index": 236,
                "h3": "8311a5fffffffff",
                "val": 0.1140164536,
                "color": "#0000ff"
            },
            {
                "level_0": 237,
                "index": 237,
                "h3": "831103fffffffff",
                "val": 0.1133212313,
                "color": "#0000ff"
            },
            {
                "level_0": 238,
                "index": 238,
                "h3": "831586fffffffff",
                "val": 0.1122783979,
                "color": "#0000fe"
            },
            {
                "level_0": 239,
                "index": 239,
                "h3": "830b15fffffffff",
                "val": 0.1118921633,
                "color": "#0000fe"
            },
            {
                "level_0": 240,
                "index": 240,
                "h3": "831112fffffffff",
                "val": 0.1107334595,
                "color": "#0000fe"
            },
            {
                "level_0": 241,
                "index": 241,
                "h3": "8310dafffffffff",
                "val": 0.1105403422,
                "color": "#0000fe"
            },
            {
                "level_0": 242,
                "index": 242,
                "h3": "830b0efffffffff",
                "val": 0.1100382372,
                "color": "#0000fe"
            },
            {
                "level_0": 243,
                "index": 243,
                "h3": "831034fffffffff",
                "val": 0.1099223668,
                "color": "#0000fe"
            },
            {
                "level_0": 244,
                "index": 244,
                "h3": "8311b4fffffffff",
                "val": 0.1095361322,
                "color": "#0000fe"
            },
            {
                "level_0": 245,
                "index": 245,
                "h3": "831582fffffffff",
                "val": 0.1089954038,
                "color": "#0000fa"
            },
            {
                "level_0": 246,
                "index": 246,
                "h3": "832065fffffffff",
                "val": 0.1085705457,
                "color": "#0000fa"
            },
            {
                "level_0": 247,
                "index": 247,
                "h3": "83206cfffffffff",
                "val": 0.1081070642,
                "color": "#0000fa"
            },
            {
                "level_0": 248,
                "index": 248,
                "h3": "83204cfffffffff",
                "val": 0.1080684408,
                "color": "#0000fa"
            },
            {
                "level_0": 249,
                "index": 249,
                "h3": "832d71fffffffff",
                "val": 0.1069869839,
                "color": "#0000fa"
            },
            {
                "level_0": 250,
                "index": 250,
                "h3": "8310b6fffffffff",
                "val": 0.1062145147,
                "color": "#0000fa"
            },
            {
                "level_0": 251,
                "index": 251,
                "h3": "8310d5fffffffff",
                "val": 0.1051330578,
                "color": "#0000f5"
            },
            {
                "level_0": 252,
                "index": 252,
                "h3": "831002fffffffff",
                "val": 0.1050558109,
                "color": "#0000f5"
            },
            {
                "level_0": 253,
                "index": 253,
                "h3": "832162fffffffff",
                "val": 0.1036653663,
                "color": "#0000f5"
            },
            {
                "level_0": 254,
                "index": 254,
                "h3": "8314d6fffffffff",
                "val": 0.103549496,
                "color": "#0000f5"
            },
            {
                "level_0": 255,
                "index": 255,
                "h3": "8310cdfffffffff",
                "val": 0.1028156502,
                "color": "#0000f5"
            },
            {
                "level_0": 256,
                "index": 256,
                "h3": "83214efffffffff",
                "val": 0.1028156502,
                "color": "#0000f5"
            },
            {
                "level_0": 257,
                "index": 257,
                "h3": "8310d0fffffffff",
                "val": 0.1019273106,
                "color": "#0000f5"
            },
            {
                "level_0": 258,
                "index": 258,
                "h3": "832535fffffffff",
                "val": 0.1018500637,
                "color": "#0000f5"
            },
            {
                "level_0": 259,
                "index": 259,
                "h3": "830134fffffffff",
                "val": 0.101116218,
                "color": "#0000f1"
            },
            {
                "level_0": 260,
                "index": 260,
                "h3": "830b91fffffffff",
                "val": 0.1005754896,
                "color": "#0000f1"
            },
            {
                "level_0": 261,
                "index": 261,
                "h3": "832d28fffffffff",
                "val": 0.1002665019,
                "color": "#0000f1"
            },
            {
                "level_0": 262,
                "index": 262,
                "h3": "830b1efffffffff",
                "val": 0.100112008,
                "color": "#0000f1"
            },
            {
                "level_0": 263,
                "index": 263,
                "h3": "832164fffffffff",
                "val": 0.0999188907,
                "color": "#0000f1"
            },
            {
                "level_0": 264,
                "index": 264,
                "h3": "832d55fffffffff",
                "val": 0.0996099031,
                "color": "#0000f1"
            },
            {
                "level_0": 265,
                "index": 265,
                "h3": "832140fffffffff",
                "val": 0.0995326561,
                "color": "#0000f1"
            },
            {
                "level_0": 266,
                "index": 266,
                "h3": "830ab4fffffffff",
                "val": 0.0976787301,
                "color": "#0000f1"
            },
            {
                "level_0": 267,
                "index": 267,
                "h3": "830b82fffffffff",
                "val": 0.0976014831,
                "color": "#0000ec"
            },
            {
                "level_0": 268,
                "index": 268,
                "h3": "831001fffffffff",
                "val": 0.0974856128,
                "color": "#0000ec"
            },
            {
                "level_0": 269,
                "index": 269,
                "h3": "831585fffffffff",
                "val": 0.0969835078,
                "color": "#0000ec"
            },
            {
                "level_0": 270,
                "index": 270,
                "h3": "831f46fffffffff",
                "val": 0.0968290139,
                "color": "#0000ec"
            },
            {
                "level_0": 271,
                "index": 271,
                "h3": "8314aafffffffff",
                "val": 0.0967903905,
                "color": "#0000ec"
            },
            {
                "level_0": 272,
                "index": 272,
                "h3": "8310b5fffffffff",
                "val": 0.0959406744,
                "color": "#0000ec"
            },
            {
                "level_0": 273,
                "index": 273,
                "h3": "8310eafffffffff",
                "val": 0.0952454521,
                "color": "#0000ec"
            },
            {
                "level_0": 274,
                "index": 274,
                "h3": "831011fffffffff",
                "val": 0.0926576803,
                "color": "#0000e8"
            },
            {
                "level_0": 275,
                "index": 275,
                "h3": "832522fffffffff",
                "val": 0.091962458,
                "color": "#0000e8"
            },
            {
                "level_0": 276,
                "index": 276,
                "h3": "83108bfffffffff",
                "val": 0.0916920938,
                "color": "#0000e8"
            },
            {
                "level_0": 277,
                "index": 277,
                "h3": "831f6cfffffffff",
                "val": 0.0914217296,
                "color": "#0000e8"
            },
            {
                "level_0": 278,
                "index": 278,
                "h3": "830a74fffffffff",
                "val": 0.0914217296,
                "color": "#0000e8"
            },
            {
                "level_0": 279,
                "index": 279,
                "h3": "832d5bfffffffff",
                "val": 0.0913444826,
                "color": "#0000e8"
            },
            {
                "level_0": 280,
                "index": 280,
                "h3": "83111efffffffff",
                "val": 0.0913058592,
                "color": "#0000e8"
            },
            {
                "level_0": 281,
                "index": 281,
                "h3": "831111fffffffff",
                "val": 0.0879842416,
                "color": "#0000e3"
            },
            {
                "level_0": 282,
                "index": 282,
                "h3": "83216bfffffffff",
                "val": 0.0879842416,
                "color": "#0000e3"
            },
            {
                "level_0": 283,
                "index": 283,
                "h3": "830b09fffffffff",
                "val": 0.0879842416,
                "color": "#0000e3"
            },
            {
                "level_0": 284,
                "index": 284,
                "h3": "830b73fffffffff",
                "val": 0.0870572786,
                "color": "#0000e3"
            },
            {
                "level_0": 285,
                "index": 285,
                "h3": "83110efffffffff",
                "val": 0.0862848094,
                "color": "#0000e3"
            },
            {
                "level_0": 286,
                "index": 286,
                "h3": "830adafffffffff",
                "val": 0.0860530686,
                "color": "#0000e3"
            },
            {
                "level_0": 287,
                "index": 287,
                "h3": "831152fffffffff",
                "val": 0.0851647291,
                "color": "#0000de"
            },
            {
                "level_0": 288,
                "index": 288,
                "h3": "8310c5fffffffff",
                "val": 0.0843922599,
                "color": "#0000de"
            },
            {
                "level_0": 289,
                "index": 289,
                "h3": "832d62fffffffff",
                "val": 0.0835039203,
                "color": "#0000de"
            },
            {
                "level_0": 290,
                "index": 290,
                "h3": "83111bfffffffff",
                "val": 0.0833880499,
                "color": "#0000de"
            },
            {
                "level_0": 291,
                "index": 291,
                "h3": "832165fffffffff",
                "val": 0.0826542042,
                "color": "#0000de"
            },
            {
                "level_0": 292,
                "index": 292,
                "h3": "832540fffffffff",
                "val": 0.0819976053,
                "color": "#0000da"
            },
            {
                "level_0": 293,
                "index": 293,
                "h3": "8310cafffffffff",
                "val": 0.0810320188,
                "color": "#0000da"
            },
            {
                "level_0": 294,
                "index": 294,
                "h3": "83101cfffffffff",
                "val": 0.0806457842,
                "color": "#0000da"
            },
            {
                "level_0": 295,
                "index": 295,
                "h3": "832edafffffffff",
                "val": 0.0804912904,
                "color": "#0000da"
            },
            {
                "level_0": 296,
                "index": 296,
                "h3": "832d70fffffffff",
                "val": 0.08037542,
                "color": "#0000da"
            },
            {
                "level_0": 297,
                "index": 297,
                "h3": "83148cfffffffff",
                "val": 0.0803367966,
                "color": "#0000da"
            },
            {
                "level_0": 298,
                "index": 298,
                "h3": "830b24fffffffff",
                "val": 0.0803367966,
                "color": "#0000da"
            },
            {
                "level_0": 299,
                "index": 299,
                "h3": "83204dfffffffff",
                "val": 0.0802595497,
                "color": "#0000da"
            },
            {
                "level_0": 300,
                "index": 300,
                "h3": "832ec9fffffffff",
                "val": 0.0785601174,
                "color": "#0000da"
            },
            {
                "level_0": 301,
                "index": 301,
                "h3": "83148bfffffffff",
                "val": 0.0767448148,
                "color": "#0000d5"
            },
            {
                "level_0": 302,
                "index": 302,
                "h3": "830b1cfffffffff",
                "val": 0.0758178518,
                "color": "#0000d5"
            },
            {
                "level_0": 303,
                "index": 303,
                "h3": "832061fffffffff",
                "val": 0.0757019814,
                "color": "#0000d5"
            },
            {
                "level_0": 304,
                "index": 304,
                "h3": "830ba3fffffffff",
                "val": 0.0748522653,
                "color": "#0000d5"
            },
            {
                "level_0": 305,
                "index": 305,
                "h3": "8315a6fffffffff",
                "val": 0.0747750183,
                "color": "#0000d5"
            },
            {
                "level_0": 306,
                "index": 306,
                "h3": "830b96fffffffff",
                "val": 0.0746977714,
                "color": "#0000d5"
            },
            {
                "level_0": 307,
                "index": 307,
                "h3": "8310d4fffffffff",
                "val": 0.0739639257,
                "color": "#0000d1"
            },
            {
                "level_0": 308,
                "index": 308,
                "h3": "832d56fffffffff",
                "val": 0.0735776911,
                "color": "#0000d1"
            },
            {
                "level_0": 309,
                "index": 309,
                "h3": "832c66fffffffff",
                "val": 0.0731914565,
                "color": "#0000d1"
            },
            {
                "level_0": 310,
                "index": 310,
                "h3": "831023fffffffff",
                "val": 0.0731142096,
                "color": "#0000d1"
            },
            {
                "level_0": 311,
                "index": 311,
                "h3": "83214afffffffff",
                "val": 0.0725348577,
                "color": "#0000d1"
            },
            {
                "level_0": 312,
                "index": 312,
                "h3": "831488fffffffff",
                "val": 0.0723803638,
                "color": "#0000d1"
            },
            {
                "level_0": 313,
                "index": 313,
                "h3": "831413fffffffff",
                "val": 0.0723417404,
                "color": "#0000d1"
            },
            {
                "level_0": 314,
                "index": 314,
                "h3": "832d50fffffffff",
                "val": 0.0721872465,
                "color": "#0000d1"
            },
            {
                "level_0": 315,
                "index": 315,
                "h3": "832166fffffffff",
                "val": 0.0712602835,
                "color": "#0000d1"
            },
            {
                "level_0": 316,
                "index": 316,
                "h3": "831025fffffffff",
                "val": 0.0705650612,
                "color": "#0000d1"
            },
            {
                "level_0": 317,
                "index": 317,
                "h3": "8310e2fffffffff",
                "val": 0.0704105674,
                "color": "#0000d1"
            },
            {
                "level_0": 318,
                "index": 318,
                "h3": "832121fffffffff",
                "val": 0.0701788266,
                "color": "#0000cc"
            },
            {
                "level_0": 319,
                "index": 319,
                "h3": "832d74fffffffff",
                "val": 0.0691746167,
                "color": "#0000cc"
            },
            {
                "level_0": 320,
                "index": 320,
                "h3": "832ecafffffffff",
                "val": 0.0691359932,
                "color": "#0000cc"
            },
            {
                "level_0": 321,
                "index": 321,
                "h3": "830a84fffffffff",
                "val": 0.0690587463,
                "color": "#0000cc"
            },
            {
                "level_0": 322,
                "index": 322,
                "h3": "832504fffffffff",
                "val": 0.0686725117,
                "color": "#0000cc"
            },
            {
                "level_0": 323,
                "index": 323,
                "h3": "831166fffffffff",
                "val": 0.0679772894,
                "color": "#0000cc"
            },
            {
                "level_0": 324,
                "index": 324,
                "h3": "831529fffffffff",
                "val": 0.0679772894,
                "color": "#0000cc"
            },
            {
                "level_0": 325,
                "index": 325,
                "h3": "8315b1fffffffff",
                "val": 0.0671275733,
                "color": "#0000cc"
            },
            {
                "level_0": 326,
                "index": 326,
                "h3": "832541fffffffff",
                "val": 0.0670117029,
                "color": "#0000cc"
            },
            {
                "level_0": 327,
                "index": 327,
                "h3": "831020fffffffff",
                "val": 0.0668958325,
                "color": "#0000cc"
            },
            {
                "level_0": 328,
                "index": 328,
                "h3": "832104fffffffff",
                "val": 0.0668958325,
                "color": "#0000cc"
            },
            {
                "level_0": 329,
                "index": 329,
                "h3": "832500fffffffff",
                "val": 0.0668958325,
                "color": "#0000cc"
            },
            {
                "level_0": 330,
                "index": 330,
                "h3": "831040fffffffff",
                "val": 0.0667799621,
                "color": "#0000cc"
            },
            {
                "level_0": 331,
                "index": 331,
                "h3": "830ab3fffffffff",
                "val": 0.0665868448,
                "color": "#0000cc"
            },
            {
                "level_0": 332,
                "index": 332,
                "h3": "832543fffffffff",
                "val": 0.0662006103,
                "color": "#0000c8"
            },
            {
                "level_0": 333,
                "index": 333,
                "h3": "83100dfffffffff",
                "val": 0.066007493,
                "color": "#0000c8"
            },
            {
                "level_0": 334,
                "index": 334,
                "h3": "8308c9fffffffff",
                "val": 0.0657371287,
                "color": "#0000c8"
            },
            {
                "level_0": 335,
                "index": 335,
                "h3": "8314aefffffffff",
                "val": 0.065505388,
                "color": "#0000c8"
            },
            {
                "level_0": 336,
                "index": 336,
                "h3": "831119fffffffff",
                "val": 0.0650805299,
                "color": "#0000c8"
            },
            {
                "level_0": 337,
                "index": 337,
                "h3": "831616fffffffff",
                "val": 0.0650419065,
                "color": "#0000c8"
            },
            {
                "level_0": 338,
                "index": 338,
                "h3": "8310dbfffffffff",
                "val": 0.0642694373,
                "color": "#0000c8"
            },
            {
                "level_0": 339,
                "index": 339,
                "h3": "830acefffffffff",
                "val": 0.06407632,
                "color": "#0000c8"
            },
            {
                "level_0": 340,
                "index": 340,
                "h3": "831003fffffffff",
                "val": 0.0640376965,
                "color": "#0000c8"
            },
            {
                "level_0": 341,
                "index": 341,
                "h3": "8314acfffffffff",
                "val": 0.0637673323,
                "color": "#0000c8"
            },
            {
                "level_0": 342,
                "index": 342,
                "h3": "832524fffffffff",
                "val": 0.0636900854,
                "color": "#0000c8"
            },
            {
                "level_0": 343,
                "index": 343,
                "h3": "83155dfffffffff",
                "val": 0.0635355915,
                "color": "#0000c8"
            },
            {
                "level_0": 344,
                "index": 344,
                "h3": "832163fffffffff",
                "val": 0.0631107335,
                "color": "#0000c8"
            },
            {
                "level_0": 345,
                "index": 345,
                "h3": "832e99fffffffff",
                "val": 0.0630334865,
                "color": "#0000c8"
            },
            {
                "level_0": 346,
                "index": 346,
                "h3": "83049bfffffffff",
                "val": 0.0627631223,
                "color": "#0000c8"
            },
            {
                "level_0": 347,
                "index": 347,
                "h3": "831008fffffffff",
                "val": 0.0623768877,
                "color": "#0000c3"
            },
            {
                "level_0": 348,
                "index": 348,
                "h3": "83142cfffffffff",
                "val": 0.0613340543,
                "color": "#0000c3"
            },
            {
                "level_0": 349,
                "index": 349,
                "h3": "832ec8fffffffff",
                "val": 0.0613340543,
                "color": "#0000c3"
            },
            {
                "level_0": 350,
                "index": 350,
                "h3": "83316dfffffffff",
                "val": 0.061140937,
                "color": "#0000c3"
            },
            {
                "level_0": 351,
                "index": 351,
                "h3": "83049cfffffffff",
                "val": 0.0608705728,
                "color": "#0000c3"
            },
            {
                "level_0": 352,
                "index": 352,
                "h3": "8314d5fffffffff",
                "val": 0.0599049863,
                "color": "#0000c3"
            },
            {
                "level_0": 353,
                "index": 353,
                "h3": "83254efffffffff",
                "val": 0.0590552702,
                "color": "#0000c3"
            },
            {
                "level_0": 354,
                "index": 354,
                "h3": "831122fffffffff",
                "val": 0.0575875787,
                "color": "#0000bf"
            },
            {
                "level_0": 355,
                "index": 355,
                "h3": "830b35fffffffff",
                "val": 0.0575489552,
                "color": "#0000bf"
            },
            {
                "level_0": 356,
                "index": 356,
                "h3": "83109cfffffffff",
                "val": 0.0574330849,
                "color": "#0000bf"
            },
            {
                "level_0": 357,
                "index": 357,
                "h3": "830b25fffffffff",
                "val": 0.0571240972,
                "color": "#0000bf"
            },
            {
                "level_0": 358,
                "index": 358,
                "h3": "832c65fffffffff",
                "val": 0.0571240972,
                "color": "#0000bf"
            },
            {
                "level_0": 359,
                "index": 359,
                "h3": "830a90fffffffff",
                "val": 0.0570082268,
                "color": "#0000bf"
            },
            {
                "level_0": 360,
                "index": 360,
                "h3": "831164fffffffff",
                "val": 0.0564674984,
                "color": "#0000bf"
            },
            {
                "level_0": 361,
                "index": 361,
                "h3": "830ba4fffffffff",
                "val": 0.0564674984,
                "color": "#0000bf"
            },
            {
                "level_0": 362,
                "index": 362,
                "h3": "830a9cfffffffff",
                "val": 0.0563902514,
                "color": "#0000bf"
            },
            {
                "level_0": 363,
                "index": 363,
                "h3": "830b55fffffffff",
                "val": 0.0558108995,
                "color": "#0000bf"
            },
            {
                "level_0": 364,
                "index": 364,
                "h3": "832533fffffffff",
                "val": 0.0549611834,
                "color": "#0000bf"
            },
            {
                "level_0": 365,
                "index": 365,
                "h3": "832ed9fffffffff",
                "val": 0.0543432081,
                "color": "#0000ba"
            },
            {
                "level_0": 366,
                "index": 366,
                "h3": "8315a0fffffffff",
                "val": 0.0538411031,
                "color": "#0000ba"
            },
            {
                "level_0": 367,
                "index": 367,
                "h3": "831715fffffffff",
                "val": 0.0537252327,
                "color": "#0000ba"
            },
            {
                "level_0": 368,
                "index": 368,
                "h3": "8310c1fffffffff",
                "val": 0.0536866092,
                "color": "#0000ba"
            },
            {
                "level_0": 369,
                "index": 369,
                "h3": "83212efffffffff",
                "val": 0.0536866092,
                "color": "#0000ba"
            },
            {
                "level_0": 370,
                "index": 370,
                "h3": "830b34fffffffff",
                "val": 0.0536866092,
                "color": "#0000ba"
            },
            {
                "level_0": 371,
                "index": 371,
                "h3": "830b54fffffffff",
                "val": 0.0536479858,
                "color": "#0000ba"
            },
            {
                "level_0": 372,
                "index": 372,
                "h3": "8311b2fffffffff",
                "val": 0.0536093623,
                "color": "#0000ba"
            },
            {
                "level_0": 373,
                "index": 373,
                "h3": "832c28fffffffff",
                "val": 0.0535321154,
                "color": "#0000ba"
            },
            {
                "level_0": 374,
                "index": 374,
                "h3": "831022fffffffff",
                "val": 0.0533776216,
                "color": "#0000ba"
            },
            {
                "level_0": 375,
                "index": 375,
                "h3": "830b85fffffffff",
                "val": 0.0527210227,
                "color": "#0000ba"
            },
            {
                "level_0": 376,
                "index": 376,
                "h3": "831054fffffffff",
                "val": 0.0526823993,
                "color": "#0000ba"
            },
            {
                "level_0": 377,
                "index": 377,
                "h3": "832069fffffffff",
                "val": 0.0519485536,
                "color": "#0000ba"
            },
            {
                "level_0": 378,
                "index": 378,
                "h3": "832120fffffffff",
                "val": 0.0517168128,
                "color": "#0000ba"
            },
            {
                "level_0": 379,
                "index": 379,
                "h3": "831105fffffffff",
                "val": 0.0514464486,
                "color": "#0000ba"
            },
            {
                "level_0": 380,
                "index": 380,
                "h3": "831045fffffffff",
                "val": 0.0512533313,
                "color": "#0000ba"
            },
            {
                "level_0": 381,
                "index": 381,
                "h3": "831583fffffffff",
                "val": 0.0501332509,
                "color": "#0000b6"
            },
            {
                "level_0": 382,
                "index": 382,
                "h3": "830b30fffffffff",
                "val": 0.0496697694,
                "color": "#0000b6"
            },
            {
                "level_0": 383,
                "index": 383,
                "h3": "831731fffffffff",
                "val": 0.0492062879,
                "color": "#0000b6"
            },
            {
                "level_0": 384,
                "index": 384,
                "h3": "830b56fffffffff",
                "val": 0.0490517941,
                "color": "#0000b6"
            },
            {
                "level_0": 385,
                "index": 385,
                "h3": "830b08fffffffff",
                "val": 0.0489745471,
                "color": "#0000b6"
            },
            {
                "level_0": 386,
                "index": 386,
                "h3": "831068fffffffff",
                "val": 0.0488586768,
                "color": "#0000b6"
            },
            {
                "level_0": 387,
                "index": 387,
                "h3": "832c72fffffffff",
                "val": 0.0483951952,
                "color": "#0000b6"
            },
            {
                "level_0": 388,
                "index": 388,
                "h3": "831116fffffffff",
                "val": 0.0482793249,
                "color": "#0000b6"
            },
            {
                "level_0": 389,
                "index": 389,
                "h3": "831e59fffffffff",
                "val": 0.0478930903,
                "color": "#0000b6"
            },
            {
                "level_0": 390,
                "index": 390,
                "h3": "830b72fffffffff",
                "val": 0.0478158433,
                "color": "#0000b6"
            },
            {
                "level_0": 391,
                "index": 391,
                "h3": "8314cafffffffff",
                "val": 0.0472751149,
                "color": "#0000b6"
            },
            {
                "level_0": 392,
                "index": 392,
                "h3": "831024fffffffff",
                "val": 0.047197868,
                "color": "#0000b6"
            },
            {
                "level_0": 393,
                "index": 393,
                "h3": "8310e8fffffffff",
                "val": 0.0471592445,
                "color": "#0000b6"
            },
            {
                "level_0": 394,
                "index": 394,
                "h3": "83104dfffffffff",
                "val": 0.0464640222,
                "color": "#0000b1"
            },
            {
                "level_0": 395,
                "index": 395,
                "h3": "8314a8fffffffff",
                "val": 0.0457688,
                "color": "#0000b1"
            },
            {
                "level_0": 396,
                "index": 396,
                "h3": "831160fffffffff",
                "val": 0.045266695,
                "color": "#0000b1"
            },
            {
                "level_0": 397,
                "index": 397,
                "h3": "8310e9fffffffff",
                "val": 0.0452280715,
                "color": "#0000b1"
            },
            {
                "level_0": 398,
                "index": 398,
                "h3": "8310e1fffffffff",
                "val": 0.0449963308,
                "color": "#0000b1"
            },
            {
                "level_0": 399,
                "index": 399,
                "h3": "831123fffffffff",
                "val": 0.0441852381,
                "color": "#0000b1"
            },
            {
                "level_0": 400,
                "index": 400,
                "h3": "830498fffffffff",
                "val": 0.0441079912,
                "color": "#0000b1"
            },
            {
                "level_0": 401,
                "index": 401,
                "h3": "83109bfffffffff",
                "val": 0.0440693677,
                "color": "#0000b1"
            },
            {
                "level_0": 402,
                "index": 402,
                "h3": "830b52fffffffff",
                "val": 0.0439921208,
                "color": "#0000b1"
            },
            {
                "level_0": 403,
                "index": 403,
                "h3": "831161fffffffff",
                "val": 0.0439148739,
                "color": "#0000b1"
            },
            {
                "level_0": 404,
                "index": 404,
                "h3": "830b98fffffffff",
                "val": 0.0435286393,
                "color": "#0000b1"
            },
            {
                "level_0": 405,
                "index": 405,
                "h3": "830b0dfffffffff",
                "val": 0.0434513924,
                "color": "#0000b1"
            },
            {
                "level_0": 406,
                "index": 406,
                "h3": "832503fffffffff",
                "val": 0.0431810282,
                "color": "#0000b1"
            },
            {
                "level_0": 407,
                "index": 407,
                "h3": "830b05fffffffff",
                "val": 0.0431424047,
                "color": "#0000b1"
            },
            {
                "level_0": 408,
                "index": 408,
                "h3": "831153fffffffff",
                "val": 0.0427561701,
                "color": "#0000ac"
            },
            {
                "level_0": 409,
                "index": 409,
                "h3": "832062fffffffff",
                "val": 0.0426016763,
                "color": "#0000ac"
            },
            {
                "level_0": 410,
                "index": 410,
                "h3": "8310eefffffffff",
                "val": 0.042331312,
                "color": "#0000ac"
            },
            {
                "level_0": 411,
                "index": 411,
                "h3": "83110dfffffffff",
                "val": 0.0420609478,
                "color": "#0000ac"
            },
            {
                "level_0": 412,
                "index": 412,
                "h3": "831580fffffffff",
                "val": 0.0419837009,
                "color": "#0000ac"
            },
            {
                "level_0": 413,
                "index": 413,
                "h3": "830a9efffffffff",
                "val": 0.0418678305,
                "color": "#0000ac"
            },
            {
                "level_0": 414,
                "index": 414,
                "h3": "830ba6fffffffff",
                "val": 0.0414815959,
                "color": "#0000ac"
            },
            {
                "level_0": 415,
                "index": 415,
                "h3": "83110afffffffff",
                "val": 0.0405546329,
                "color": "#0000ac"
            },
            {
                "level_0": 416,
                "index": 416,
                "h3": "832506fffffffff",
                "val": 0.040477386,
                "color": "#0000ac"
            },
            {
                "level_0": 417,
                "index": 417,
                "h3": "831528fffffffff",
                "val": 0.0402070217,
                "color": "#0000ac"
            },
            {
                "level_0": 418,
                "index": 418,
                "h3": "830b33fffffffff",
                "val": 0.039975281,
                "color": "#0000ac"
            },
            {
                "level_0": 419,
                "index": 419,
                "h3": "831176fffffffff",
                "val": 0.0396276698,
                "color": "#0000ac"
            },
            {
                "level_0": 420,
                "index": 420,
                "h3": "8310cefffffffff",
                "val": 0.0388938241,
                "color": "#0000a8"
            },
            {
                "level_0": 421,
                "index": 421,
                "h3": "832168fffffffff",
                "val": 0.0387007068,
                "color": "#0000a8"
            },
            {
                "level_0": 422,
                "index": 422,
                "h3": "830a2cfffffffff",
                "val": 0.0386234599,
                "color": "#0000a8"
            },
            {
                "level_0": 423,
                "index": 423,
                "h3": "831101fffffffff",
                "val": 0.0381213549,
                "color": "#0000a8"
            },
            {
                "level_0": 424,
                "index": 424,
                "h3": "831595fffffffff",
                "val": 0.0379668611,
                "color": "#0000a8"
            },
            {
                "level_0": 425,
                "index": 425,
                "h3": "831051fffffffff",
                "val": 0.0377737438,
                "color": "#0000a8"
            },
            {
                "level_0": 426,
                "index": 426,
                "h3": "830b2cfffffffff",
                "val": 0.0374647561,
                "color": "#0000a8"
            },
            {
                "level_0": 427,
                "index": 427,
                "h3": "832143fffffffff",
                "val": 0.0372330153,
                "color": "#0000a8"
            },
            {
                "level_0": 428,
                "index": 428,
                "h3": "830baefffffffff",
                "val": 0.0371943919,
                "color": "#0000a8"
            },
            {
                "level_0": 429,
                "index": 429,
                "h3": "832574fffffffff",
                "val": 0.0371557684,
                "color": "#0000a8"
            },
            {
                "level_0": 430,
                "index": 430,
                "h3": "831174fffffffff",
                "val": 0.0371557684,
                "color": "#0000a8"
            },
            {
                "level_0": 431,
                "index": 431,
                "h3": "832d63fffffffff",
                "val": 0.0369240277,
                "color": "#0000a8"
            },
            {
                "level_0": 432,
                "index": 432,
                "h3": "831423fffffffff",
                "val": 0.0368467807,
                "color": "#0000a8"
            },
            {
                "level_0": 433,
                "index": 433,
                "h3": "831175fffffffff",
                "val": 0.0367695338,
                "color": "#0000a8"
            },
            {
                "level_0": 434,
                "index": 434,
                "h3": "831e4afffffffff",
                "val": 0.0365764165,
                "color": "#0000a8"
            },
            {
                "level_0": 435,
                "index": 435,
                "h3": "8310c9fffffffff",
                "val": 0.0365377931,
                "color": "#0000a8"
            },
            {
                "level_0": 436,
                "index": 436,
                "h3": "832542fffffffff",
                "val": 0.0362674288,
                "color": "#0000a8"
            },
            {
                "level_0": 437,
                "index": 437,
                "h3": "831170fffffffff",
                "val": 0.0359584412,
                "color": "#0000a8"
            },
            {
                "level_0": 438,
                "index": 438,
                "h3": "8304ebfffffffff",
                "val": 0.0358811942,
                "color": "#0000a8"
            },
            {
                "level_0": 439,
                "index": 439,
                "h3": "832159fffffffff",
                "val": 0.03561083,
                "color": "#0000a8"
            },
            {
                "level_0": 440,
                "index": 440,
                "h3": "831433fffffffff",
                "val": 0.0352245954,
                "color": "#0000a8"
            },
            {
                "level_0": 441,
                "index": 441,
                "h3": "832e8bfffffffff",
                "val": 0.035185972,
                "color": "#0000a8"
            },
            {
                "level_0": 442,
                "index": 442,
                "h3": "830a29fffffffff",
                "val": 0.0350701016,
                "color": "#0000a3"
            },
            {
                "level_0": 443,
                "index": 443,
                "h3": "83110cfffffffff",
                "val": 0.0346066201,
                "color": "#0000a3"
            },
            {
                "level_0": 444,
                "index": 444,
                "h3": "832ecdfffffffff",
                "val": 0.0343748793,
                "color": "#0000a3"
            },
            {
                "level_0": 445,
                "index": 445,
                "h3": "830b9bfffffffff",
                "val": 0.0342976324,
                "color": "#0000a3"
            },
            {
                "level_0": 446,
                "index": 446,
                "h3": "830b01fffffffff",
                "val": 0.0342590089,
                "color": "#0000a3"
            },
            {
                "level_0": 447,
                "index": 447,
                "h3": "831552fffffffff",
                "val": 0.0339886447,
                "color": "#0000a3"
            },
            {
                "level_0": 448,
                "index": 448,
                "h3": "832544fffffffff",
                "val": 0.0338341509,
                "color": "#0000a3"
            },
            {
                "level_0": 449,
                "index": 449,
                "h3": "830b16fffffffff",
                "val": 0.0337569039,
                "color": "#0000a3"
            },
            {
                "level_0": 450,
                "index": 450,
                "h3": "83316efffffffff",
                "val": 0.0336410336,
                "color": "#0000a3"
            },
            {
                "level_0": 451,
                "index": 451,
                "h3": "832ec1fffffffff",
                "val": 0.0333706693,
                "color": "#0000a3"
            },
            {
                "level_0": 452,
                "index": 452,
                "h3": "832570fffffffff",
                "val": 0.0333320459,
                "color": "#0000a3"
            },
            {
                "level_0": 453,
                "index": 453,
                "h3": "830aa6fffffffff",
                "val": 0.0332934224,
                "color": "#0000a3"
            },
            {
                "level_0": 454,
                "index": 454,
                "h3": "832c09fffffffff",
                "val": 0.033254799,
                "color": "#0000a3"
            },
            {
                "level_0": 455,
                "index": 455,
                "h3": "8314dbfffffffff",
                "val": 0.0331003051,
                "color": "#0000a3"
            },
            {
                "level_0": 456,
                "index": 456,
                "h3": "832546fffffffff",
                "val": 0.0330616817,
                "color": "#0000a3"
            },
            {
                "level_0": 457,
                "index": 457,
                "h3": "832520fffffffff",
                "val": 0.0327140705,
                "color": "#0000a3"
            },
            {
                "level_0": 458,
                "index": 458,
                "h3": "832175fffffffff",
                "val": 0.0325982001,
                "color": "#0000a3"
            },
            {
                "level_0": 459,
                "index": 459,
                "h3": "830b50fffffffff",
                "val": 0.0324050828,
                "color": "#0000a3"
            },
            {
                "level_0": 460,
                "index": 460,
                "h3": "831006fffffffff",
                "val": 0.0321733421,
                "color": "#0000a3"
            },
            {
                "level_0": 461,
                "index": 461,
                "h3": "8314d9fffffffff",
                "val": 0.0321733421,
                "color": "#0000a3"
            },
            {
                "level_0": 462,
                "index": 462,
                "h3": "831136fffffffff",
                "val": 0.0320960952,
                "color": "#0000a3"
            },
            {
                "level_0": 463,
                "index": 463,
                "h3": "831431fffffffff",
                "val": 0.0320188482,
                "color": "#0000a3"
            },
            {
                "level_0": 464,
                "index": 464,
                "h3": "83105bfffffffff",
                "val": 0.0317871075,
                "color": "#0000a3"
            },
            {
                "level_0": 465,
                "index": 465,
                "h3": "830a9afffffffff",
                "val": 0.0315939902,
                "color": "#0000a3"
            },
            {
                "level_0": 466,
                "index": 466,
                "h3": "831574fffffffff",
                "val": 0.0308987679,
                "color": "#00009f"
            },
            {
                "level_0": 467,
                "index": 467,
                "h3": "830b03fffffffff",
                "val": 0.0308601445,
                "color": "#00009f"
            },
            {
                "level_0": 468,
                "index": 468,
                "h3": "831575fffffffff",
                "val": 0.0304739099,
                "color": "#00009f"
            },
            {
                "level_0": 469,
                "index": 469,
                "h3": "830b76fffffffff",
                "val": 0.0301649222,
                "color": "#00009f"
            },
            {
                "level_0": 470,
                "index": 470,
                "h3": "830ab0fffffffff",
                "val": 0.0300104283,
                "color": "#00009f"
            },
            {
                "level_0": 471,
                "index": 471,
                "h3": "831e6dfffffffff",
                "val": 0.029392453,
                "color": "#00009f"
            },
            {
                "level_0": 472,
                "index": 472,
                "h3": "83102dfffffffff",
                "val": 0.0292379591,
                "color": "#00009f"
            },
            {
                "level_0": 473,
                "index": 473,
                "h3": "831055fffffffff",
                "val": 0.0288517245,
                "color": "#00009f"
            },
            {
                "level_0": 474,
                "index": 474,
                "h3": "831704fffffffff",
                "val": 0.0287358542,
                "color": "#00009f"
            },
            {
                "level_0": 475,
                "index": 475,
                "h3": "832eccfffffffff",
                "val": 0.0285041134,
                "color": "#00009f"
            },
            {
                "level_0": 476,
                "index": 476,
                "h3": "832e8afffffffff",
                "val": 0.0282337492,
                "color": "#00009f"
            },
            {
                "level_0": 477,
                "index": 477,
                "h3": "832545fffffffff",
                "val": 0.027963385,
                "color": "#00009f"
            },
            {
                "level_0": 478,
                "index": 478,
                "h3": "830a82fffffffff",
                "val": 0.0277316442,
                "color": "#00009f"
            },
            {
                "level_0": 479,
                "index": 479,
                "h3": "83106cfffffffff",
                "val": 0.0273067861,
                "color": "#00009a"
            },
            {
                "level_0": 480,
                "index": 480,
                "h3": "831421fffffffff",
                "val": 0.0272681627,
                "color": "#00009a"
            },
            {
                "level_0": 481,
                "index": 481,
                "h3": "831464fffffffff",
                "val": 0.0270364219,
                "color": "#00009a"
            },
            {
                "level_0": 482,
                "index": 482,
                "h3": "830addfffffffff",
                "val": 0.026959175,
                "color": "#00009a"
            },
            {
                "level_0": 483,
                "index": 483,
                "h3": "83111dfffffffff",
                "val": 0.0267274342,
                "color": "#00009a"
            },
            {
                "level_0": 484,
                "index": 484,
                "h3": "830a80fffffffff",
                "val": 0.0263798231,
                "color": "#00009a"
            },
            {
                "level_0": 485,
                "index": 485,
                "h3": "83102efffffffff",
                "val": 0.0258777181,
                "color": "#00009a"
            },
            {
                "level_0": 486,
                "index": 486,
                "h3": "83100cfffffffff",
                "val": 0.0256846008,
                "color": "#00009a"
            },
            {
                "level_0": 487,
                "index": 487,
                "h3": "831403fffffffff",
                "val": 0.0256459774,
                "color": "#00009a"
            },
            {
                "level_0": 488,
                "index": 488,
                "h3": "830aa2fffffffff",
                "val": 0.0253756131,
                "color": "#00009a"
            },
            {
                "level_0": 489,
                "index": 489,
                "h3": "830ba5fffffffff",
                "val": 0.0251438724,
                "color": "#00009a"
            },
            {
                "level_0": 490,
                "index": 490,
                "h3": "830ad8fffffffff",
                "val": 0.0249121316,
                "color": "#00009a"
            },
            {
                "level_0": 491,
                "index": 491,
                "h3": "830b0afffffffff",
                "val": 0.0248348847,
                "color": "#00009a"
            },
            {
                "level_0": 492,
                "index": 492,
                "h3": "831021fffffffff",
                "val": 0.0246417674,
                "color": "#00009a"
            },
            {
                "level_0": 493,
                "index": 493,
                "h3": "831026fffffffff",
                "val": 0.024525897,
                "color": "#00009a"
            },
            {
                "level_0": 494,
                "index": 494,
                "h3": "83255cfffffffff",
                "val": 0.0243327797,
                "color": "#00009a"
            },
            {
                "level_0": 495,
                "index": 495,
                "h3": "831559fffffffff",
                "val": 0.0242941563,
                "color": "#00009a"
            },
            {
                "level_0": 496,
                "index": 496,
                "h3": "831004fffffffff",
                "val": 0.0242555328,
                "color": "#00009a"
            },
            {
                "level_0": 497,
                "index": 497,
                "h3": "830b1dfffffffff",
                "val": 0.0242555328,
                "color": "#00009a"
            },
            {
                "level_0": 498,
                "index": 498,
                "h3": "831000fffffffff",
                "val": 0.0241782859,
                "color": "#00009a"
            },
            {
                "level_0": 499,
                "index": 499,
                "h3": "8315b4fffffffff",
                "val": 0.0241396624,
                "color": "#00009a"
            },
            {
                "level_0": 500,
                "index": 500,
                "h3": "831071fffffffff",
                "val": 0.0240237921,
                "color": "#00009a"
            },
            {
                "level_0": 501,
                "index": 501,
                "h3": "8315aefffffffff",
                "val": 0.0238306748,
                "color": "#00009a"
            },
            {
                "level_0": 502,
                "index": 502,
                "h3": "832125fffffffff",
                "val": 0.0237534278,
                "color": "#00009a"
            },
            {
                "level_0": 503,
                "index": 503,
                "h3": "83102cfffffffff",
                "val": 0.0237534278,
                "color": "#00009a"
            },
            {
                "level_0": 504,
                "index": 504,
                "h3": "83101dfffffffff",
                "val": 0.0236761809,
                "color": "#00009a"
            },
            {
                "level_0": 505,
                "index": 505,
                "h3": "83141afffffffff",
                "val": 0.0229809586,
                "color": "#000096"
            },
            {
                "level_0": 506,
                "index": 506,
                "h3": "831056fffffffff",
                "val": 0.0229423352,
                "color": "#000096"
            },
            {
                "level_0": 507,
                "index": 507,
                "h3": "832d52fffffffff",
                "val": 0.0228650883,
                "color": "#000096"
            },
            {
                "level_0": 508,
                "index": 508,
                "h3": "83152efffffffff",
                "val": 0.0227105944,
                "color": "#000096"
            },
            {
                "level_0": 509,
                "index": 509,
                "h3": "832563fffffffff",
                "val": 0.0224402302,
                "color": "#000096"
            },
            {
                "level_0": 510,
                "index": 510,
                "h3": "832066fffffffff",
                "val": 0.0223243598,
                "color": "#000096"
            },
            {
                "level_0": 511,
                "index": 511,
                "h3": "8315b5fffffffff",
                "val": 0.0223243598,
                "color": "#000096"
            },
            {
                "level_0": 512,
                "index": 512,
                "h3": "830b92fffffffff",
                "val": 0.022169866,
                "color": "#000096"
            },
            {
                "level_0": 513,
                "index": 513,
                "h3": "832c0dfffffffff",
                "val": 0.0220153721,
                "color": "#000096"
            },
            {
                "level_0": 514,
                "index": 514,
                "h3": "831f49fffffffff",
                "val": 0.0219767487,
                "color": "#000096"
            },
            {
                "level_0": 515,
                "index": 515,
                "h3": "831404fffffffff",
                "val": 0.0219381252,
                "color": "#000096"
            },
            {
                "level_0": 516,
                "index": 516,
                "h3": "832530fffffffff",
                "val": 0.0219381252,
                "color": "#000096"
            },
            {
                "level_0": 517,
                "index": 517,
                "h3": "831041fffffffff",
                "val": 0.0217450079,
                "color": "#000096"
            },
            {
                "level_0": 518,
                "index": 518,
                "h3": "830b2bfffffffff",
                "val": 0.0217063845,
                "color": "#000096"
            },
            {
                "level_0": 519,
                "index": 519,
                "h3": "83154afffffffff",
                "val": 0.021667761,
                "color": "#000096"
            },
            {
                "level_0": 520,
                "index": 520,
                "h3": "831735fffffffff",
                "val": 0.0215518906,
                "color": "#000096"
            },
            {
                "level_0": 521,
                "index": 521,
                "h3": "831733fffffffff",
                "val": 0.0213587733,
                "color": "#000096"
            },
            {
                "level_0": 522,
                "index": 522,
                "h3": "831154fffffffff",
                "val": 0.0213587733,
                "color": "#000096"
            },
            {
                "level_0": 523,
                "index": 523,
                "h3": "831031fffffffff",
                "val": 0.0212429029,
                "color": "#000096"
            },
            {
                "level_0": 524,
                "index": 524,
                "h3": "832525fffffffff",
                "val": 0.0211270326,
                "color": "#000096"
            },
            {
                "level_0": 525,
                "index": 525,
                "h3": "830b21fffffffff",
                "val": 0.0211270326,
                "color": "#000096"
            },
            {
                "level_0": 526,
                "index": 526,
                "h3": "831073fffffffff",
                "val": 0.0210884091,
                "color": "#000096"
            },
            {
                "level_0": 527,
                "index": 527,
                "h3": "830b74fffffffff",
                "val": 0.0209725387,
                "color": "#000096"
            },
            {
                "level_0": 528,
                "index": 528,
                "h3": "830ac8fffffffff",
                "val": 0.0208566683,
                "color": "#000096"
            },
            {
                "level_0": 529,
                "index": 529,
                "h3": "830b90fffffffff",
                "val": 0.0208566683,
                "color": "#000096"
            },
            {
                "level_0": 530,
                "index": 530,
                "h3": "830a94fffffffff",
                "val": 0.020740798,
                "color": "#000096"
            },
            {
                "level_0": 531,
                "index": 531,
                "h3": "831046fffffffff",
                "val": 0.020740798,
                "color": "#000096"
            },
            {
                "level_0": 532,
                "index": 532,
                "h3": "8314adfffffffff",
                "val": 0.0207021745,
                "color": "#000096"
            },
            {
                "level_0": 533,
                "index": 533,
                "h3": "831465fffffffff",
                "val": 0.0205863041,
                "color": "#000096"
            },
            {
                "level_0": 534,
                "index": 534,
                "h3": "831f72fffffffff",
                "val": 0.0202773164,
                "color": "#000096"
            },
            {
                "level_0": 535,
                "index": 535,
                "h3": "832c75fffffffff",
                "val": 0.0202000695,
                "color": "#000096"
            },
            {
                "level_0": 536,
                "index": 536,
                "h3": "831f68fffffffff",
                "val": 0.0200841991,
                "color": "#000096"
            },
            {
                "level_0": 537,
                "index": 537,
                "h3": "831566fffffffff",
                "val": 0.0200069522,
                "color": "#000096"
            },
            {
                "level_0": 538,
                "index": 538,
                "h3": "832ecefffffffff",
                "val": 0.0199297053,
                "color": "#000096"
            },
            {
                "level_0": 539,
                "index": 539,
                "h3": "830b10fffffffff",
                "val": 0.0198138349,
                "color": "#000096"
            },
            {
                "level_0": 540,
                "index": 540,
                "h3": "83254cfffffffff",
                "val": 0.0198138349,
                "color": "#000096"
            },
            {
                "level_0": 541,
                "index": 541,
                "h3": "830b9cfffffffff",
                "val": 0.0196979645,
                "color": "#000096"
            },
            {
                "level_0": 542,
                "index": 542,
                "h3": "8314c8fffffffff",
                "val": 0.0195820942,
                "color": "#000096"
            },
            {
                "level_0": 543,
                "index": 543,
                "h3": "8315a4fffffffff",
                "val": 0.0193117299,
                "color": "#000091"
            },
            {
                "level_0": 544,
                "index": 544,
                "h3": "832c62fffffffff",
                "val": 0.0192731065,
                "color": "#000091"
            },
            {
                "level_0": 545,
                "index": 545,
                "h3": "83102afffffffff",
                "val": 0.0192731065,
                "color": "#000091"
            },
            {
                "level_0": 546,
                "index": 546,
                "h3": "8314defffffffff",
                "val": 0.0191572361,
                "color": "#000091"
            },
            {
                "level_0": 547,
                "index": 547,
                "h3": "83215dfffffffff",
                "val": 0.0191572361,
                "color": "#000091"
            },
            {
                "level_0": 548,
                "index": 548,
                "h3": "832573fffffffff",
                "val": 0.0186937546,
                "color": "#000091"
            },
            {
                "level_0": 549,
                "index": 549,
                "h3": "832158fffffffff",
                "val": 0.0185392607,
                "color": "#000091"
            },
            {
                "level_0": 550,
                "index": 550,
                "h3": "831563fffffffff",
                "val": 0.0183847669,
                "color": "#000091"
            },
            {
                "level_0": 551,
                "index": 551,
                "h3": "831100fffffffff",
                "val": 0.0183847669,
                "color": "#000091"
            },
            {
                "level_0": 552,
                "index": 552,
                "h3": "831543fffffffff",
                "val": 0.0183461434,
                "color": "#000091"
            },
            {
                "level_0": 553,
                "index": 553,
                "h3": "830b18fffffffff",
                "val": 0.0182688965,
                "color": "#000091"
            },
            {
                "level_0": 554,
                "index": 554,
                "h3": "831049fffffffff",
                "val": 0.0182688965,
                "color": "#000091"
            },
            {
                "level_0": 555,
                "index": 555,
                "h3": "832144fffffffff",
                "val": 0.0182302731,
                "color": "#000091"
            },
            {
                "level_0": 556,
                "index": 556,
                "h3": "832549fffffffff",
                "val": 0.0180371558,
                "color": "#000091"
            },
            {
                "level_0": 557,
                "index": 557,
                "h3": "831063fffffffff",
                "val": 0.0176895446,
                "color": "#000091"
            },
            {
                "level_0": 558,
                "index": 558,
                "h3": "83252dfffffffff",
                "val": 0.0175736742,
                "color": "#000091"
            },
            {
                "level_0": 559,
                "index": 559,
                "h3": "8314cdfffffffff",
                "val": 0.0175736742,
                "color": "#000091"
            },
            {
                "level_0": 560,
                "index": 560,
                "h3": "832171fffffffff",
                "val": 0.0174964273,
                "color": "#000091"
            },
            {
                "level_0": 561,
                "index": 561,
                "h3": "830135fffffffff",
                "val": 0.0173419335,
                "color": "#000091"
            },
            {
                "level_0": 562,
                "index": 562,
                "h3": "831730fffffffff",
                "val": 0.01730331,
                "color": "#000091"
            },
            {
                "level_0": 563,
                "index": 563,
                "h3": "831134fffffffff",
                "val": 0.01730331,
                "color": "#000091"
            },
            {
                "level_0": 564,
                "index": 564,
                "h3": "8315a8fffffffff",
                "val": 0.0172646866,
                "color": "#000091"
            },
            {
                "level_0": 565,
                "index": 565,
                "h3": "830ab1fffffffff",
                "val": 0.0169943224,
                "color": "#000091"
            },
            {
                "level_0": 566,
                "index": 566,
                "h3": "831521fffffffff",
                "val": 0.0168012051,
                "color": "#000091"
            },
            {
                "level_0": 567,
                "index": 567,
                "h3": "83254dfffffffff",
                "val": 0.0166080878,
                "color": "#000091"
            },
            {
                "level_0": 568,
                "index": 568,
                "h3": "831172fffffffff",
                "val": 0.0166080878,
                "color": "#000091"
            },
            {
                "level_0": 569,
                "index": 569,
                "h3": "830b6efffffffff",
                "val": 0.0164535939,
                "color": "#000091"
            },
            {
                "level_0": 570,
                "index": 570,
                "h3": "830aa0fffffffff",
                "val": 0.016376347,
                "color": "#000091"
            },
            {
                "level_0": 571,
                "index": 571,
                "h3": "831714fffffffff",
                "val": 0.0163377235,
                "color": "#000091"
            },
            {
                "level_0": 572,
                "index": 572,
                "h3": "831156fffffffff",
                "val": 0.0163377235,
                "color": "#000091"
            },
            {
                "level_0": 573,
                "index": 573,
                "h3": "832100fffffffff",
                "val": 0.0162604766,
                "color": "#000091"
            },
            {
                "level_0": 574,
                "index": 574,
                "h3": "832531fffffffff",
                "val": 0.0161832297,
                "color": "#000091"
            },
            {
                "level_0": 575,
                "index": 575,
                "h3": "83112bfffffffff",
                "val": 0.0161832297,
                "color": "#000091"
            },
            {
                "level_0": 576,
                "index": 576,
                "h3": "83155efffffffff",
                "val": 0.0157197482,
                "color": "#000091"
            },
            {
                "level_0": 577,
                "index": 577,
                "h3": "831118fffffffff",
                "val": 0.0156038778,
                "color": "#00008d"
            },
            {
                "level_0": 578,
                "index": 578,
                "h3": "830ba1fffffffff",
                "val": 0.015449384,
                "color": "#00008d"
            },
            {
                "level_0": 579,
                "index": 579,
                "h3": "832161fffffffff",
                "val": 0.0154107605,
                "color": "#00008d"
            },
            {
                "level_0": 580,
                "index": 580,
                "h3": "8308c8fffffffff",
                "val": 0.0152176432,
                "color": "#00008d"
            },
            {
                "level_0": 581,
                "index": 581,
                "h3": "831072fffffffff",
                "val": 0.0151790197,
                "color": "#00008d"
            },
            {
                "level_0": 582,
                "index": 582,
                "h3": "832572fffffffff",
                "val": 0.0151017728,
                "color": "#00008d"
            },
            {
                "level_0": 583,
                "index": 583,
                "h3": "831410fffffffff",
                "val": 0.0150631494,
                "color": "#00008d"
            },
            {
                "level_0": 584,
                "index": 584,
                "h3": "830b13fffffffff",
                "val": 0.0150631494,
                "color": "#00008d"
            },
            {
                "level_0": 585,
                "index": 585,
                "h3": "830b29fffffffff",
                "val": 0.0150245259,
                "color": "#00008d"
            },
            {
                "level_0": 586,
                "index": 586,
                "h3": "830b81fffffffff",
                "val": 0.0149859024,
                "color": "#00008d"
            },
            {
                "level_0": 587,
                "index": 587,
                "h3": "832d61fffffffff",
                "val": 0.014947279,
                "color": "#00008d"
            },
            {
                "level_0": 588,
                "index": 588,
                "h3": "831098fffffffff",
                "val": 0.0145996678,
                "color": "#00008d"
            },
            {
                "level_0": 589,
                "index": 589,
                "h3": "831572fffffffff",
                "val": 0.0145610444,
                "color": "#00008d"
            },
            {
                "level_0": 590,
                "index": 590,
                "h3": "83212afffffffff",
                "val": 0.0145224209,
                "color": "#00008d"
            },
            {
                "level_0": 591,
                "index": 591,
                "h3": "832128fffffffff",
                "val": 0.014445174,
                "color": "#00008d"
            },
            {
                "level_0": 592,
                "index": 592,
                "h3": "832560fffffffff",
                "val": 0.0143679271,
                "color": "#00008d"
            },
            {
                "level_0": 593,
                "index": 593,
                "h3": "83216cfffffffff",
                "val": 0.0143293036,
                "color": "#00008d"
            },
            {
                "level_0": 594,
                "index": 594,
                "h3": "831135fffffffff",
                "val": 0.0142906802,
                "color": "#00008d"
            },
            {
                "level_0": 595,
                "index": 595,
                "h3": "83255dfffffffff",
                "val": 0.0142520567,
                "color": "#00008d"
            },
            {
                "level_0": 596,
                "index": 596,
                "h3": "831562fffffffff",
                "val": 0.0142520567,
                "color": "#00008d"
            },
            {
                "level_0": 597,
                "index": 597,
                "h3": "831108fffffffff",
                "val": 0.0141748098,
                "color": "#00008d"
            },
            {
                "level_0": 598,
                "index": 598,
                "h3": "831722fffffffff",
                "val": 0.0140589394,
                "color": "#00008d"
            },
            {
                "level_0": 599,
                "index": 599,
                "h3": "8308d8fffffffff",
                "val": 0.0140203159,
                "color": "#00008d"
            },
            {
                "level_0": 600,
                "index": 600,
                "h3": "8308cbfffffffff",
                "val": 0.0139044456,
                "color": "#00008d"
            },
            {
                "level_0": 601,
                "index": 601,
                "h3": "831533fffffffff",
                "val": 0.0139044456,
                "color": "#00008d"
            },
            {
                "level_0": 602,
                "index": 602,
                "h3": "831520fffffffff",
                "val": 0.0138658221,
                "color": "#00008d"
            },
            {
                "level_0": 603,
                "index": 603,
                "h3": "832101fffffffff",
                "val": 0.0138271986,
                "color": "#00008d"
            },
            {
                "level_0": 604,
                "index": 604,
                "h3": "831050fffffffff",
                "val": 0.0137885752,
                "color": "#00008d"
            },
            {
                "level_0": 605,
                "index": 605,
                "h3": "8310ccfffffffff",
                "val": 0.0137499517,
                "color": "#00008d"
            },
            {
                "level_0": 606,
                "index": 606,
                "h3": "83178bfffffffff",
                "val": 0.0137113283,
                "color": "#00008d"
            },
            {
                "level_0": 607,
                "index": 607,
                "h3": "83151dfffffffff",
                "val": 0.0137113283,
                "color": "#00008d"
            },
            {
                "level_0": 608,
                "index": 608,
                "h3": "832169fffffffff",
                "val": 0.0135954579,
                "color": "#00008d"
            },
            {
                "level_0": 609,
                "index": 609,
                "h3": "831426fffffffff",
                "val": 0.0135568344,
                "color": "#00008d"
            },
            {
                "level_0": 610,
                "index": 610,
                "h3": "831019fffffffff",
                "val": 0.013518211,
                "color": "#00008d"
            },
            {
                "level_0": 611,
                "index": 611,
                "h3": "830b46fffffffff",
                "val": 0.0133637171,
                "color": "#00008d"
            },
            {
                "level_0": 612,
                "index": 612,
                "h3": "831402fffffffff",
                "val": 0.0133250937,
                "color": "#00008d"
            },
            {
                "level_0": 613,
                "index": 613,
                "h3": "830a98fffffffff",
                "val": 0.0130547294,
                "color": "#00008d"
            },
            {
                "level_0": 614,
                "index": 614,
                "h3": "8314c2fffffffff",
                "val": 0.0128616121,
                "color": "#00008d"
            },
            {
                "level_0": 615,
                "index": 615,
                "h3": "831502fffffffff",
                "val": 0.0127071183,
                "color": "#00008d"
            },
            {
                "level_0": 616,
                "index": 616,
                "h3": "83256dfffffffff",
                "val": 0.0126298714,
                "color": "#00008d"
            },
            {
                "level_0": 617,
                "index": 617,
                "h3": "830a9bfffffffff",
                "val": 0.0124753775,
                "color": "#00008d"
            },
            {
                "level_0": 618,
                "index": 618,
                "h3": "831523fffffffff",
                "val": 0.0124753775,
                "color": "#00008d"
            },
            {
                "level_0": 619,
                "index": 619,
                "h3": "8314c6fffffffff",
                "val": 0.0123208837,
                "color": "#00008d"
            },
            {
                "level_0": 620,
                "index": 620,
                "h3": "83116efffffffff",
                "val": 0.0123208837,
                "color": "#00008d"
            },
            {
                "level_0": 621,
                "index": 621,
                "h3": "8310c8fffffffff",
                "val": 0.0121663899,
                "color": "#00008d"
            },
            {
                "level_0": 622,
                "index": 622,
                "h3": "830a28fffffffff",
                "val": 0.0121277664,
                "color": "#00008d"
            },
            {
                "level_0": 623,
                "index": 623,
                "h3": "830a95fffffffff",
                "val": 0.0118574022,
                "color": "#00008d"
            },
            {
                "level_0": 624,
                "index": 624,
                "h3": "830b99fffffffff",
                "val": 0.0118574022,
                "color": "#00008d"
            },
            {
                "level_0": 625,
                "index": 625,
                "h3": "831581fffffffff",
                "val": 0.0118187787,
                "color": "#00008d"
            },
            {
                "level_0": 626,
                "index": 626,
                "h3": "831109fffffffff",
                "val": 0.0116256614,
                "color": "#000088"
            },
            {
                "level_0": 627,
                "index": 627,
                "h3": "832129fffffffff",
                "val": 0.0116256614,
                "color": "#000088"
            },
            {
                "level_0": 628,
                "index": 628,
                "h3": "83115efffffffff",
                "val": 0.011587038,
                "color": "#000088"
            },
            {
                "level_0": 629,
                "index": 629,
                "h3": "83216dfffffffff",
                "val": 0.011587038,
                "color": "#000088"
            },
            {
                "level_0": 630,
                "index": 630,
                "h3": "831519fffffffff",
                "val": 0.011509791,
                "color": "#000088"
            },
            {
                "level_0": 631,
                "index": 631,
                "h3": "8315a9fffffffff",
                "val": 0.0114711676,
                "color": "#000088"
            },
            {
                "level_0": 632,
                "index": 632,
                "h3": "831554fffffffff",
                "val": 0.0114325441,
                "color": "#000088"
            },
            {
                "level_0": 633,
                "index": 633,
                "h3": "830ba2fffffffff",
                "val": 0.0114325441,
                "color": "#000088"
            },
            {
                "level_0": 634,
                "index": 634,
                "h3": "832174fffffffff",
                "val": 0.0113166737,
                "color": "#000088"
            },
            {
                "level_0": 635,
                "index": 635,
                "h3": "831515fffffffff",
                "val": 0.0112780503,
                "color": "#000088"
            },
            {
                "level_0": 636,
                "index": 636,
                "h3": "831430fffffffff",
                "val": 0.0112780503,
                "color": "#000088"
            },
            {
                "level_0": 637,
                "index": 637,
                "h3": "830aa4fffffffff",
                "val": 0.0112008034,
                "color": "#000088"
            },
            {
                "level_0": 638,
                "index": 638,
                "h3": "831734fffffffff",
                "val": 0.0110463095,
                "color": "#000088"
            },
            {
                "level_0": 639,
                "index": 639,
                "h3": "831558fffffffff",
                "val": 0.0109304391,
                "color": "#000088"
            },
            {
                "level_0": 640,
                "index": 640,
                "h3": "832edbfffffffff",
                "val": 0.0108531922,
                "color": "#000088"
            },
            {
                "level_0": 641,
                "index": 641,
                "h3": "832131fffffffff",
                "val": 0.0108145688,
                "color": "#000088"
            },
            {
                "level_0": 642,
                "index": 642,
                "h3": "831091fffffffff",
                "val": 0.0107759453,
                "color": "#000088"
            },
            {
                "level_0": 643,
                "index": 643,
                "h3": "831436fffffffff",
                "val": 0.0107373218,
                "color": "#000088"
            },
            {
                "level_0": 644,
                "index": 644,
                "h3": "830b32fffffffff",
                "val": 0.0106986984,
                "color": "#000088"
            },
            {
                "level_0": 645,
                "index": 645,
                "h3": "831042fffffffff",
                "val": 0.0106600749,
                "color": "#000088"
            },
            {
                "level_0": 646,
                "index": 646,
                "h3": "831460fffffffff",
                "val": 0.010582828,
                "color": "#000088"
            },
            {
                "level_0": 647,
                "index": 647,
                "h3": "832502fffffffff",
                "val": 0.010582828,
                "color": "#000088"
            },
            {
                "level_0": 648,
                "index": 648,
                "h3": "831005fffffffff",
                "val": 0.0103897107,
                "color": "#000088"
            },
            {
                "level_0": 649,
                "index": 649,
                "h3": "831f54fffffffff",
                "val": 0.0103897107,
                "color": "#000088"
            },
            {
                "level_0": 650,
                "index": 650,
                "h3": "8308d9fffffffff",
                "val": 0.0103124638,
                "color": "#000088"
            },
            {
                "level_0": 651,
                "index": 651,
                "h3": "830b14fffffffff",
                "val": 0.0102738403,
                "color": "#000088"
            },
            {
                "level_0": 652,
                "index": 652,
                "h3": "830bacfffffffff",
                "val": 0.0102738403,
                "color": "#000088"
            },
            {
                "level_0": 653,
                "index": 653,
                "h3": "831f6bfffffffff",
                "val": 0.0102352169,
                "color": "#000088"
            },
            {
                "level_0": 654,
                "index": 654,
                "h3": "830b28fffffffff",
                "val": 0.0102352169,
                "color": "#000088"
            },
            {
                "level_0": 655,
                "index": 655,
                "h3": "831466fffffffff",
                "val": 0.0101965934,
                "color": "#000088"
            },
            {
                "level_0": 656,
                "index": 656,
                "h3": "831705fffffffff",
                "val": 0.0100034761,
                "color": "#000088"
            },
            {
                "level_0": 657,
                "index": 657,
                "h3": "8314abfffffffff",
                "val": 0.0098876057,
                "color": "#000088"
            },
            {
                "level_0": 658,
                "index": 658,
                "h3": "831432fffffffff",
                "val": 0.0098489823,
                "color": "#000088"
            },
            {
                "level_0": 659,
                "index": 659,
                "h3": "832ed8fffffffff",
                "val": 0.0097717354,
                "color": "#000088"
            },
            {
                "level_0": 660,
                "index": 660,
                "h3": "832548fffffffff",
                "val": 0.0094627477,
                "color": "#000088"
            },
            {
                "level_0": 661,
                "index": 661,
                "h3": "831129fffffffff",
                "val": 0.0094627477,
                "color": "#000088"
            },
            {
                "level_0": 662,
                "index": 662,
                "h3": "8314d4fffffffff",
                "val": 0.0094241242,
                "color": "#000088"
            },
            {
                "level_0": 663,
                "index": 663,
                "h3": "832eddfffffffff",
                "val": 0.0093468773,
                "color": "#000088"
            },
            {
                "level_0": 664,
                "index": 664,
                "h3": "8308dbfffffffff",
                "val": 0.0091923835,
                "color": "#000088"
            },
            {
                "level_0": 665,
                "index": 665,
                "h3": "831553fffffffff",
                "val": 0.00915376,
                "color": "#000088"
            },
            {
                "level_0": 666,
                "index": 666,
                "h3": "830b0cfffffffff",
                "val": 0.0090765131,
                "color": "#000088"
            },
            {
                "level_0": 667,
                "index": 667,
                "h3": "831536fffffffff",
                "val": 0.0090378896,
                "color": "#000088"
            },
            {
                "level_0": 668,
                "index": 668,
                "h3": "8315aafffffffff",
                "val": 0.0089992662,
                "color": "#000088"
            },
            {
                "level_0": 669,
                "index": 669,
                "h3": "830ab5fffffffff",
                "val": 0.0089220192,
                "color": "#000088"
            },
            {
                "level_0": 670,
                "index": 670,
                "h3": "831e49fffffffff",
                "val": 0.0087289019,
                "color": "#000088"
            },
            {
                "level_0": 671,
                "index": 671,
                "h3": "830136fffffffff",
                "val": 0.0085357846,
                "color": "#000088"
            },
            {
                "level_0": 672,
                "index": 672,
                "h3": "830ab6fffffffff",
                "val": 0.0084199143,
                "color": "#000088"
            },
            {
                "level_0": 673,
                "index": 673,
                "h3": "831706fffffffff",
                "val": 0.0082654204,
                "color": "#000088"
            },
            {
                "level_0": 674,
                "index": 674,
                "h3": "831f55fffffffff",
                "val": 0.008226797,
                "color": "#000088"
            },
            {
                "level_0": 675,
                "index": 675,
                "h3": "831630fffffffff",
                "val": 0.00814955,
                "color": "#000088"
            },
            {
                "level_0": 676,
                "index": 676,
                "h3": "830db6fffffffff",
                "val": 0.00814955,
                "color": "#000088"
            },
            {
                "level_0": 677,
                "index": 677,
                "h3": "831400fffffffff",
                "val": 0.00814955,
                "color": "#000088"
            },
            {
                "level_0": 678,
                "index": 678,
                "h3": "832c2cfffffffff",
                "val": 0.0081109266,
                "color": "#000088"
            },
            {
                "level_0": 679,
                "index": 679,
                "h3": "831415fffffffff",
                "val": 0.0080723031,
                "color": "#000088"
            },
            {
                "level_0": 680,
                "index": 680,
                "h3": "8310e4fffffffff",
                "val": 0.0079950562,
                "color": "#000088"
            },
            {
                "level_0": 681,
                "index": 681,
                "h3": "832eacfffffffff",
                "val": 0.0079950562,
                "color": "#000088"
            },
            {
                "level_0": 682,
                "index": 682,
                "h3": "83151cfffffffff",
                "val": 0.0078405624,
                "color": "#000088"
            },
            {
                "level_0": 683,
                "index": 683,
                "h3": "8314f6fffffffff",
                "val": 0.0078405624,
                "color": "#000088"
            },
            {
                "level_0": 684,
                "index": 684,
                "h3": "831f48fffffffff",
                "val": 0.0078019389,
                "color": "#000084"
            },
            {
                "level_0": 685,
                "index": 685,
                "h3": "830b2afffffffff",
                "val": 0.007724692,
                "color": "#000084"
            },
            {
                "level_0": 686,
                "index": 686,
                "h3": "830b0bfffffffff",
                "val": 0.007724692,
                "color": "#000084"
            },
            {
                "level_0": 687,
                "index": 687,
                "h3": "831789fffffffff",
                "val": 0.0076860685,
                "color": "#000084"
            },
            {
                "level_0": 688,
                "index": 688,
                "h3": "830b83fffffffff",
                "val": 0.0074543278,
                "color": "#000084"
            },
            {
                "level_0": 689,
                "index": 689,
                "h3": "830aa5fffffffff",
                "val": 0.0074543278,
                "color": "#000084"
            },
            {
                "level_0": 690,
                "index": 690,
                "h3": "830b12fffffffff",
                "val": 0.0073770808,
                "color": "#000084"
            },
            {
                "level_0": 691,
                "index": 691,
                "h3": "831028fffffffff",
                "val": 0.0072998339,
                "color": "#000084"
            },
            {
                "level_0": 692,
                "index": 692,
                "h3": "831009fffffffff",
                "val": 0.0072612105,
                "color": "#000084"
            },
            {
                "level_0": 693,
                "index": 693,
                "h3": "832555fffffffff",
                "val": 0.0072612105,
                "color": "#000084"
            },
            {
                "level_0": 694,
                "index": 694,
                "h3": "830a2dfffffffff",
                "val": 0.0071067166,
                "color": "#000084"
            },
            {
                "level_0": 695,
                "index": 695,
                "h3": "83150cfffffffff",
                "val": 0.0069908462,
                "color": "#000084"
            },
            {
                "level_0": 696,
                "index": 696,
                "h3": "830b23fffffffff",
                "val": 0.0069522228,
                "color": "#000084"
            },
            {
                "level_0": 697,
                "index": 697,
                "h3": "831145fffffffff",
                "val": 0.0069135993,
                "color": "#000084"
            },
            {
                "level_0": 698,
                "index": 698,
                "h3": "830accfffffffff",
                "val": 0.0068749759,
                "color": "#000084"
            },
            {
                "level_0": 699,
                "index": 699,
                "h3": "83104cfffffffff",
                "val": 0.0068749759,
                "color": "#000084"
            },
            {
                "level_0": 700,
                "index": 700,
                "h3": "8314e6fffffffff",
                "val": 0.0068363524,
                "color": "#000084"
            },
            {
                "level_0": 701,
                "index": 701,
                "h3": "8310e0fffffffff",
                "val": 0.006720482,
                "color": "#000084"
            },
            {
                "level_0": 702,
                "index": 702,
                "h3": "8304cefffffffff",
                "val": 0.0066432351,
                "color": "#000084"
            },
            {
                "level_0": 703,
                "index": 703,
                "h3": "831522fffffffff",
                "val": 0.0066432351,
                "color": "#000084"
            },
            {
                "level_0": 704,
                "index": 704,
                "h3": "8304c8fffffffff",
                "val": 0.0066046116,
                "color": "#000084"
            },
            {
                "level_0": 705,
                "index": 705,
                "h3": "832044fffffffff",
                "val": 0.0064887413,
                "color": "#000084"
            },
            {
                "level_0": 706,
                "index": 706,
                "h3": "832c2bfffffffff",
                "val": 0.0064114943,
                "color": "#000084"
            },
            {
                "level_0": 707,
                "index": 707,
                "h3": "832566fffffffff",
                "val": 0.006295624,
                "color": "#000084"
            },
            {
                "level_0": 708,
                "index": 708,
                "h3": "832534fffffffff",
                "val": 0.0062570005,
                "color": "#000084"
            },
            {
                "level_0": 709,
                "index": 709,
                "h3": "83115afffffffff",
                "val": 0.0062570005,
                "color": "#000084"
            },
            {
                "level_0": 710,
                "index": 710,
                "h3": "8315abfffffffff",
                "val": 0.006218377,
                "color": "#000084"
            },
            {
                "level_0": 711,
                "index": 711,
                "h3": "831059fffffffff",
                "val": 0.0061797536,
                "color": "#000084"
            },
            {
                "level_0": 712,
                "index": 712,
                "h3": "8314dafffffffff",
                "val": 0.0060252597,
                "color": "#000084"
            },
            {
                "level_0": 713,
                "index": 713,
                "h3": "831564fffffffff",
                "val": 0.0059480128,
                "color": "#000084"
            },
            {
                "level_0": 714,
                "index": 714,
                "h3": "830adefffffffff",
                "val": 0.0059093894,
                "color": "#000084"
            },
            {
                "level_0": 715,
                "index": 715,
                "h3": "832e89fffffffff",
                "val": 0.0059093894,
                "color": "#000084"
            },
            {
                "level_0": 716,
                "index": 716,
                "h3": "83154bfffffffff",
                "val": 0.0059093894,
                "color": "#000084"
            },
            {
                "level_0": 717,
                "index": 717,
                "h3": "83141bfffffffff",
                "val": 0.0058707659,
                "color": "#000084"
            },
            {
                "level_0": 718,
                "index": 718,
                "h3": "830a86fffffffff",
                "val": 0.0057548955,
                "color": "#000084"
            },
            {
                "level_0": 719,
                "index": 719,
                "h3": "831096fffffffff",
                "val": 0.0057162721,
                "color": "#000084"
            },
            {
                "level_0": 720,
                "index": 720,
                "h3": "832c63fffffffff",
                "val": 0.0056776486,
                "color": "#000084"
            },
            {
                "level_0": 721,
                "index": 721,
                "h3": "832562fffffffff",
                "val": 0.0056776486,
                "color": "#000084"
            },
            {
                "level_0": 722,
                "index": 722,
                "h3": "831422fffffffff",
                "val": 0.0056390251,
                "color": "#000084"
            },
            {
                "level_0": 723,
                "index": 723,
                "h3": "830ba0fffffffff",
                "val": 0.0055617782,
                "color": "#000084"
            },
            {
                "level_0": 724,
                "index": 724,
                "h3": "8304cdfffffffff",
                "val": 0.0054072844,
                "color": "#000084"
            },
            {
                "level_0": 725,
                "index": 725,
                "h3": "83256bfffffffff",
                "val": 0.0053686609,
                "color": "#000084"
            },
            {
                "level_0": 726,
                "index": 726,
                "h3": "83152afffffffff",
                "val": 0.0053686609,
                "color": "#000084"
            },
            {
                "level_0": 727,
                "index": 727,
                "h3": "831500fffffffff",
                "val": 0.005291414,
                "color": "#000084"
            },
            {
                "level_0": 728,
                "index": 728,
                "h3": "8308dafffffffff",
                "val": 0.0052141671,
                "color": "#000084"
            },
            {
                "level_0": 729,
                "index": 729,
                "h3": "831532fffffffff",
                "val": 0.0051755436,
                "color": "#000084"
            },
            {
                "level_0": 730,
                "index": 730,
                "h3": "831534fffffffff",
                "val": 0.0050982967,
                "color": "#000084"
            },
            {
                "level_0": 731,
                "index": 731,
                "h3": "831525fffffffff",
                "val": 0.0050210498,
                "color": "#000084"
            },
            {
                "level_0": 732,
                "index": 732,
                "h3": "83100efffffffff",
                "val": 0.0049824263,
                "color": "#000084"
            },
            {
                "level_0": 733,
                "index": 733,
                "h3": "8315a1fffffffff",
                "val": 0.0049051794,
                "color": "#000084"
            },
            {
                "level_0": 734,
                "index": 734,
                "h3": "8310e6fffffffff",
                "val": 0.0048665559,
                "color": "#000084"
            },
            {
                "level_0": 735,
                "index": 735,
                "h3": "831076fffffffff",
                "val": 0.004789309,
                "color": "#000084"
            },
            {
                "level_0": 736,
                "index": 736,
                "h3": "8308cafffffffff",
                "val": 0.0046734386,
                "color": "#000084"
            },
            {
                "level_0": 737,
                "index": 737,
                "h3": "8305a2fffffffff",
                "val": 0.0046348152,
                "color": "#000084"
            },
            {
                "level_0": 738,
                "index": 738,
                "h3": "8314f1fffffffff",
                "val": 0.0045961917,
                "color": "#000084"
            },
            {
                "level_0": 739,
                "index": 739,
                "h3": "83112dfffffffff",
                "val": 0.0045961917,
                "color": "#000084"
            },
            {
                "level_0": 740,
                "index": 740,
                "h3": "83110bfffffffff",
                "val": 0.0045575683,
                "color": "#000084"
            },
            {
                "level_0": 741,
                "index": 741,
                "h3": "830ab2fffffffff",
                "val": 0.0045575683,
                "color": "#000084"
            },
            {
                "level_0": 742,
                "index": 742,
                "h3": "831f4afffffffff",
                "val": 0.0044803213,
                "color": "#000084"
            },
            {
                "level_0": 743,
                "index": 743,
                "h3": "831511fffffffff",
                "val": 0.0044803213,
                "color": "#000084"
            },
            {
                "level_0": 744,
                "index": 744,
                "h3": "830a2bfffffffff",
                "val": 0.0044803213,
                "color": "#000084"
            },
            {
                "level_0": 745,
                "index": 745,
                "h3": "831155fffffffff",
                "val": 0.0044416979,
                "color": "#000084"
            },
            {
                "level_0": 746,
                "index": 746,
                "h3": "832e8cfffffffff",
                "val": 0.0044030744,
                "color": "#000084"
            },
            {
                "level_0": 747,
                "index": 747,
                "h3": "831075fffffffff",
                "val": 0.0044030744,
                "color": "#000084"
            },
            {
                "level_0": 748,
                "index": 748,
                "h3": "831530fffffffff",
                "val": 0.0043258275,
                "color": "#000084"
            },
            {
                "level_0": 749,
                "index": 749,
                "h3": "832eebfffffffff",
                "val": 0.0043258275,
                "color": "#000084"
            },
            {
                "level_0": 750,
                "index": 750,
                "h3": "8308ddfffffffff",
                "val": 0.0042485806,
                "color": "#000084"
            },
            {
                "level_0": 751,
                "index": 751,
                "h3": "830b53fffffffff",
                "val": 0.0042485806,
                "color": "#000084"
            },
            {
                "level_0": 752,
                "index": 752,
                "h3": "831411fffffffff",
                "val": 0.0042485806,
                "color": "#000084"
            },
            {
                "level_0": 753,
                "index": 753,
                "h3": "83140cfffffffff",
                "val": 0.0041327102,
                "color": "#000084"
            },
            {
                "level_0": 754,
                "index": 754,
                "h3": "83112afffffffff",
                "val": 0.0039782164,
                "color": "#000084"
            },
            {
                "level_0": 755,
                "index": 755,
                "h3": "832045fffffffff",
                "val": 0.0038237225,
                "color": "#00007f"
            },
            {
                "level_0": 756,
                "index": 756,
                "h3": "831570fffffffff",
                "val": 0.0037464756,
                "color": "#00007f"
            },
            {
                "level_0": 757,
                "index": 757,
                "h3": "830b36fffffffff",
                "val": 0.0037464756,
                "color": "#00007f"
            },
            {
                "level_0": 758,
                "index": 758,
                "h3": "8304ddfffffffff",
                "val": 0.0037078521,
                "color": "#00007f"
            },
            {
                "level_0": 759,
                "index": 759,
                "h3": "83102bfffffffff",
                "val": 0.0036306052,
                "color": "#00007f"
            },
            {
                "level_0": 760,
                "index": 760,
                "h3": "830a9dfffffffff",
                "val": 0.0035533583,
                "color": "#00007f"
            },
            {
                "level_0": 761,
                "index": 761,
                "h3": "832532fffffffff",
                "val": 0.0034761114,
                "color": "#00007f"
            },
            {
                "level_0": 762,
                "index": 762,
                "h3": "831754fffffffff",
                "val": 0.0034374879,
                "color": "#00007f"
            },

            {
                "level_0": 764,
                "index": 764,
                "h3": "83089bfffffffff",
                "val": 0.0034374879,
                "color": "#00007f"
            },
            {
                "level_0": 765,
                "index": 765,
                "h3": "831614fffffffff",
                "val": 0.0033988645,
                "color": "#00007f"
            },
            {
                "level_0": 766,
                "index": 766,
                "h3": "832d60fffffffff",
                "val": 0.003360241,
                "color": "#00007f"
            },
            {
                "level_0": 767,
                "index": 767,
                "h3": "831633fffffffff",
                "val": 0.0032829941,
                "color": "#00007f"
            },
            {
                "level_0": 768,
                "index": 768,
                "h3": "8315b3fffffffff",
                "val": 0.0032443706,
                "color": "#00007f"
            },
            {
                "level_0": 769,
                "index": 769,
                "h3": "8314f2fffffffff",
                "val": 0.0032057472,
                "color": "#00007f"
            },
            {
                "level_0": 770,
                "index": 770,
                "h3": "83141cfffffffff",
                "val": 0.0032057472,
                "color": "#00007f"
            },
            {
                "level_0": 771,
                "index": 771,
                "h3": "831f71fffffffff",
                "val": 0.0031671237,
                "color": "#00007f"
            },
            {
                "level_0": 772,
                "index": 772,
                "h3": "831526fffffffff",
                "val": 0.0030898768,
                "color": "#00007f"
            },
            {
                "level_0": 773,
                "index": 773,
                "h3": "83152bfffffffff",
                "val": 0.0030898768,
                "color": "#00007f"
            },
            {
                "level_0": 774,
                "index": 774,
                "h3": "830490fffffffff",
                "val": 0.0029740064,
                "color": "#00007f"
            },
            {
                "level_0": 775,
                "index": 775,
                "h3": "830b2dfffffffff",
                "val": 0.0029740064,
                "color": "#00007f"
            },
            {
                "level_0": 776,
                "index": 776,
                "h3": "8314dcfffffffff",
                "val": 0.0029740064,
                "color": "#00007f"
            },
            {
                "level_0": 777,
                "index": 777,
                "h3": "832515fffffffff",
                "val": 0.002935383,
                "color": "#00007f"
            },
            {
                "level_0": 778,
                "index": 778,
                "h3": "831710fffffffff",
                "val": 0.0028967595,
                "color": "#00007f"
            },
            {
                "level_0": 779,
                "index": 779,
                "h3": "831f40fffffffff",
                "val": 0.0028195126,
                "color": "#00007f"
            },
            {
                "level_0": 780,
                "index": 780,
                "h3": "83106dfffffffff",
                "val": 0.0025105249,
                "color": "#00007f"
            },
            {
                "level_0": 781,
                "index": 781,
                "h3": "832eeafffffffff",
                "val": 0.0024719014,
                "color": "#00007f"
            },
            {
                "level_0": 782,
                "index": 782,
                "h3": "831f4dfffffffff",
                "val": 0.002433278,
                "color": "#00007f"
            },
            {
                "level_0": 783,
                "index": 783,
                "h3": "831162fffffffff",
                "val": 0.002433278,
                "color": "#00007f"
            },
            {
                "level_0": 784,
                "index": 784,
                "h3": "831444fffffffff",
                "val": 0.002433278,
                "color": "#00007f"
            },
            {
                "level_0": 785,
                "index": 785,
                "h3": "83154efffffffff",
                "val": 0.0023946545,
                "color": "#00007f"
            },
            {
                "level_0": 786,
                "index": 786,
                "h3": "832536fffffffff",
                "val": 0.0023946545,
                "color": "#00007f"
            },
            {
                "level_0": 787,
                "index": 787,
                "h3": "83155cfffffffff",
                "val": 0.0023560311,
                "color": "#00007f"
            },
            {
                "level_0": 788,
                "index": 788,
                "h3": "8314eafffffffff",
                "val": 0.0023560311,
                "color": "#00007f"
            },
            {
                "level_0": 789,
                "index": 789,
                "h3": "83216efffffffff",
                "val": 0.0021629138,
                "color": "#00007f"
            },
            {
                "level_0": 790,
                "index": 790,
                "h3": "831576fffffffff",
                "val": 0.0020856668,
                "color": "#00007f"
            },
            {
                "level_0": 791,
                "index": 791,
                "h3": "831535fffffffff",
                "val": 0.0020084199,
                "color": "#00007f"
            },
            {
                "level_0": 792,
                "index": 792,
                "h3": "831061fffffffff",
                "val": 0.0020084199,
                "color": "#00007f"
            },
            {
                "level_0": 793,
                "index": 793,
                "h3": "831560fffffffff",
                "val": 0.001931173,
                "color": "#00007f"
            },
            {
                "level_0": 794,
                "index": 794,
                "h3": "831414fffffffff",
                "val": 0.001931173,
                "color": "#00007f"
            },
            {
                "level_0": 795,
                "index": 795,
                "h3": "8310cbfffffffff",
                "val": 0.001931173,
                "color": "#00007f"
            },
            {
                "level_0": 796,
                "index": 796,
                "h3": "831406fffffffff",
                "val": 0.0018925495,
                "color": "#00007f"
            },
            {
                "level_0": 797,
                "index": 797,
                "h3": "831501fffffffff",
                "val": 0.0017766792,
                "color": "#00007f"
            },
            {
                "level_0": 798,
                "index": 798,
                "h3": "830491fffffffff",
                "val": 0.0017380557,
                "color": "#00007f"
            },
            {
                "level_0": 799,
                "index": 799,
                "h3": "831171fffffffff",
                "val": 0.0017380557,
                "color": "#00007f"
            },
            {
                "level_0": 800,
                "index": 800,
                "h3": "8304c0fffffffff",
                "val": 0.0017380557,
                "color": "#00007f"
            },
            {
                "level_0": 801,
                "index": 801,
                "h3": "831489fffffffff",
                "val": 0.0016994322,
                "color": "#00007f"
            },
            {
                "level_0": 802,
                "index": 802,
                "h3": "8305b5fffffffff",
                "val": 0.0016608088,
                "color": "#00007f"
            },
            {
                "level_0": 803,
                "index": 803,
                "h3": "831632fffffffff",
                "val": 0.0016608088,
                "color": "#00007f"
            },
            {
                "level_0": 804,
                "index": 804,
                "h3": "83148afffffffff",
                "val": 0.0016608088,
                "color": "#00007f"
            },
            {
                "level_0": 805,
                "index": 805,
                "h3": "83158dfffffffff",
                "val": 0.0016608088,
                "color": "#00007f"
            },
            {
                "level_0": 806,
                "index": 806,
                "h3": "832554fffffffff",
                "val": 0.0016221853,
                "color": "#00007f"
            },
            {
                "level_0": 807,
                "index": 807,
                "h3": "830b5bfffffffff",
                "val": 0.0015835619,
                "color": "#00007f"
            },
            {
                "level_0": 808,
                "index": 808,
                "h3": "83100afffffffff",
                "val": 0.0015449384,
                "color": "#00007f"
            },
            {
                "level_0": 809,
                "index": 809,
                "h3": "831e48fffffffff",
                "val": 0.0015449384,
                "color": "#00007f"
            },
            {
                "level_0": 810,
                "index": 810,
                "h3": "83152dfffffffff",
                "val": 0.0015063149,
                "color": "#00007f"
            },
            {
                "level_0": 811,
                "index": 811,
                "h3": "831435fffffffff",
                "val": 0.0014676915,
                "color": "#00007f"
            },
            {
                "level_0": 812,
                "index": 812,
                "h3": "830a24fffffffff",
                "val": 0.0014676915,
                "color": "#00007f"
            },
            {
                "level_0": 813,
                "index": 813,
                "h3": "8316a5fffffffff",
                "val": 0.001429068,
                "color": "#00007f"
            },
            {
                "level_0": 814,
                "index": 814,
                "h3": "831128fffffffff",
                "val": 0.0013904446,
                "color": "#00007f"
            },
            {
                "level_0": 815,
                "index": 815,
                "h3": "83256afffffffff",
                "val": 0.0013518211,
                "color": "#00007f"
            },
            {
                "level_0": 816,
                "index": 816,
                "h3": "83048bfffffffff",
                "val": 0.0013518211,
                "color": "#00007f"
            },
            {
                "level_0": 817,
                "index": 817,
                "h3": "830d99fffffffff",
                "val": 0.0013518211,
                "color": "#00007f"
            },
            {
                "level_0": 818,
                "index": 818,
                "h3": "83155bfffffffff",
                "val": 0.0012745742,
                "color": "#00007f"
            },
            {
                "level_0": 819,
                "index": 819,
                "h3": "8311b6fffffffff",
                "val": 0.0012359507,
                "color": "#00007f"
            },
            {
                "level_0": 820,
                "index": 820,
                "h3": "831f6efffffffff",
                "val": 0.0012359507,
                "color": "#00007f"
            },
            {
                "level_0": 821,
                "index": 821,
                "h3": "8304b5fffffffff",
                "val": 0.0011973273,
                "color": "#00007f"
            },
            {
                "level_0": 822,
                "index": 822,
                "h3": "830483fffffffff",
                "val": 0.0011973273,
                "color": "#00007f"
            },
            {
                "level_0": 823,
                "index": 823,
                "h3": "830b2efffffffff",
                "val": 0.0011587038,
                "color": "#00007f"
            },
            {
                "level_0": 824,
                "index": 824,
                "h3": "8314a1fffffffff",
                "val": 0.0011200803,
                "color": "#00007f"
            },
            {
                "level_0": 825,
                "index": 825,
                "h3": "8314c5fffffffff",
                "val": 0.0010814569,
                "color": "#00007f"
            },
            {
                "level_0": 826,
                "index": 826,
                "h3": "83106efffffffff",
                "val": 0.0010428334,
                "color": "#00007f"
            },
            {
                "level_0": 827,
                "index": 827,
                "h3": "831504fffffffff",
                "val": 0.0010428334,
                "color": "#00007f"
            },
            {
                "level_0": 828,
                "index": 828,
                "h3": "832568fffffffff",
                "val": 0.0010428334,
                "color": "#00007f"
            },
            {
                "level_0": 829,
                "index": 829,
                "h3": "83100bfffffffff",
                "val": 0.00100421,
                "color": "#00007f"
            },
            {
                "level_0": 830,
                "index": 830,
                "h3": "8304ccfffffffff",
                "val": 0.0009655865,
                "color": "#00007f"
            },
            {
                "level_0": 831,
                "index": 831,
                "h3": "8304c1fffffffff",
                "val": 0.0009655865,
                "color": "#00007f"
            },
            {
                "level_0": 832,
                "index": 832,
                "h3": "832501fffffffff",
                "val": 0.0009655865,
                "color": "#00007f"
            },
            {
                "level_0": 833,
                "index": 833,
                "h3": "8314e8fffffffff",
                "val": 0.000926963,
                "color": "#00007f"
            },
            {
                "level_0": 834,
                "index": 834,
                "h3": "831463fffffffff",
                "val": 0.0008883396,
                "color": "#00007f"
            },
            {
                "level_0": 835,
                "index": 835,
                "h3": "8314f0fffffffff",
                "val": 0.0008883396,
                "color": "#00007f"
            },
            {
                "level_0": 836,
                "index": 836,
                "h3": "83141efffffffff",
                "val": 0.0008497161,
                "color": "#00007f"
            },
            {
                "level_0": 837,
                "index": 837,
                "h3": "831029fffffffff",
                "val": 0.0008110927,
                "color": "#00007f"
            },
            {
                "level_0": 838,
                "index": 838,
                "h3": "832e12fffffffff",
                "val": 0.0008110927,
                "color": "#00007f"
            },
            {
                "level_0": 839,
                "index": 839,
                "h3": "830d82fffffffff",
                "val": 0.0007338457,
                "color": "#00007f"
            },
            {
                "level_0": 840,
                "index": 840,
                "h3": "830badfffffffff",
                "val": 0.0007338457,
                "color": "#00007f"
            },
            {
                "level_0": 841,
                "index": 841,
                "h3": "830b19fffffffff",
                "val": 0.0006952223,
                "color": "#00007f"
            },
            {
                "level_0": 842,
                "index": 842,
                "h3": "831548fffffffff",
                "val": 0.0006179754,
                "color": "#00007f"
            },
            {
                "level_0": 843,
                "index": 843,
                "h3": "83250dfffffffff",
                "val": 0.0006179754,
                "color": "#00007f"
            },
            {
                "level_0": 844,
                "index": 844,
                "h3": "830a25fffffffff",
                "val": 0.0005793519,
                "color": "#00007f"
            },
            {
                "level_0": 845,
                "index": 845,
                "h3": "831424fffffffff",
                "val": 0.0005793519,
                "color": "#00007f"
            },
            {
                "level_0": 846,
                "index": 846,
                "h3": "83104bfffffffff",
                "val": 0.0005407284,
                "color": "#00007f"
            },
            {
                "level_0": 847,
                "index": 847,
                "h3": "831604fffffffff",
                "val": 0.000502105,
                "color": "#00007f"
            },
            {
                "level_0": 848,
                "index": 848,
                "h3": "8310e5fffffffff",
                "val": 0.000502105,
                "color": "#00007f"
            },
            {
                "level_0": 849,
                "index": 849,
                "h3": "83148dfffffffff",
                "val": 0.0004634815,
                "color": "#00007f"
            },
            {
                "level_0": 850,
                "index": 850,
                "h3": "8314ccfffffffff",
                "val": 0.0004248581,
                "color": "#00007f"
            },
            {
                "level_0": 851,
                "index": 851,
                "h3": "832135fffffffff",
                "val": 0.0004248581,
                "color": "#00007f"
            },
            {
                "level_0": 852,
                "index": 852,
                "h3": "831556fffffffff",
                "val": 0.0004248581,
                "color": "#00007f"
            },
            {
                "level_0": 853,
                "index": 853,
                "h3": "832d75fffffffff",
                "val": 0.0004248581,
                "color": "#00007f"
            },
            {
                "level_0": 854,
                "index": 854,
                "h3": "831462fffffffff",
                "val": 0.0004248581,
                "color": "#00007f"
            },
            {
                "level_0": 855,
                "index": 855,
                "h3": "831043fffffffff",
                "val": 0.0003862346,
                "color": "#00007f"
            },
            {
                "level_0": 856,
                "index": 856,
                "h3": "831545fffffffff",
                "val": 0.0003862346,
                "color": "#00007f"
            },
            {
                "level_0": 857,
                "index": 857,
                "h3": "8314a9fffffffff",
                "val": 0.0003476111,
                "color": "#00007f"
            },
            {
                "level_0": 858,
                "index": 858,
                "h3": "831418fffffffff",
                "val": 0.0003089877,
                "color": "#00007f"
            },
            {
                "level_0": 859,
                "index": 859,
                "h3": "832550fffffffff",
                "val": 0.0003089877,
                "color": "#00007f"
            },
            {
                "level_0": 860,
                "index": 860,
                "h3": "832ec2fffffffff",
                "val": 0.0002703642,
                "color": "#00007f"
            },
            {
                "level_0": 861,
                "index": 861,
                "h3": "830a83fffffffff",
                "val": 0.0002703642,
                "color": "#00007f"
            },
            {
                "level_0": 862,
                "index": 862,
                "h3": "830b1afffffffff",
                "val": 0.0002703642,
                "color": "#00007f"
            },
            {
                "level_0": 863,
                "index": 863,
                "h3": "831505fffffffff",
                "val": 0.0001931173,
                "color": "#00007f"
            },
            {
                "level_0": 864,
                "index": 864,
                "h3": "8314f3fffffffff",
                "val": 0.0001931173,
                "color": "#00007f"
            },
            {
                "level_0": 865,
                "index": 865,
                "h3": "8314f4fffffffff",
                "val": 0.0001931173,
                "color": "#00007f"
            },
            {
                "level_0": 866,
                "index": 866,
                "h3": "831612fffffffff",
                "val": 0.0001544938,
                "color": "#00007f"
            },
            {
                "level_0": 867,
                "index": 867,
                "h3": "831e6cfffffffff",
                "val": 0.0001544938,
                "color": "#00007f"
            },
            {
                "level_0": 868,
                "index": 868,
                "h3": "830a93fffffffff",
                "val": 0.0001544938,
                "color": "#00007f"
            },
            {
                "level_0": 869,
                "index": 869,
                "h3": "8314d8fffffffff",
                "val": 0.0001158704,
                "color": "#00007f"
            },
            {
                "level_0": 870,
                "index": 870,
                "h3": "830d88fffffffff",
                "val": 0.0001158704,
                "color": "#00007f"
            },
            {
                "level_0": 871,
                "index": 871,
                "h3": "830588fffffffff",
                "val": 0.0001158704,
                "color": "#00007f"
            },
            {
                "level_0": 872,
                "index": 872,
                "h3": "83049efffffffff",
                "val": 0.0001158704,
                "color": "#00007f"
            },
            {
                "level_0": 873,
                "index": 873,
                "h3": "8314d1fffffffff",
                "val": 7.72469e-05,
                "color": "#00007f"
            },
            {
                "level_0": 874,
                "index": 874,
                "h3": "8314c3fffffffff",
                "val": 7.72469e-05,
                "color": "#00007f"
            },
            {
                "level_0": 875,
                "index": 875,
                "h3": "83115cfffffffff",
                "val": 7.72469e-05,
                "color": "#00007f"
            },
            {
                "level_0": 876,
                "index": 876,
                "h3": "83148efffffffff",
                "val": 3.86235e-05,
                "color": "#00007f"
            },
            {
                "level_0": 877,
                "index": 877,
                "h3": "830adcfffffffff",
                "val": 0.0,
                "color": "#00007f"
            },
            {
                "level_0": 878,
                "index": 878,
                "h3": "83150efffffffff",
                "val": 0.0,
                "color": "#00007f"
            }
        ]
        };
    const h3s = geo['geo'].map((data) => {
        let hexagonMap = new Map()
        const hexCount = hexagonMap.get(data.h3);
        if (hexCount) {
            hexagonMap.set(data.h3, hexCount + 1);
        } else {
            hexagonMap.set(data.h3, 1);
        }

        Array.from(hexagonMap.keys()).forEach((key) => {
            const kring = h3.gridRingUnsafe(key, 0);
            kring.forEach((hex) => {
                if (!hexagonMap.has(hex)) {
                    hexagonMap.set(hex, 0);
                }
            });
        });

        const [counts, keys] = [
            Array.from(hexagonMap.values()),
            Array.from(hexagonMap.keys())
        ];

        let max = Math.max(...counts);
        let min = Math.min(...counts);

        let hexagons = keys.map((key, i) => {
            let polygon = h3.cellToBoundary(key, false);
            return [polygon, getWeight(counts[i], min, max)];
        });
        return groupPolygonsByOpacity(hexagons);
    });
    const geoH3 = h3s.map((data, index) => {
        return {
            id: geo['geo'][index].index,
            h3: data,
            val: geo['geo'][index].val,
            color: geo['geo'][index].color
        }
    })
    return {geoH3};
}
